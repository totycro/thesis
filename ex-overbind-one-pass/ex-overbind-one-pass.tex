...
\largered{all combinations of quantifiers appear to work, arrows meaningless}
...
\begin{prooftree}
	...
	\BinaryInfCm{ P(f(x)) \mid R(x) \lor Q(x_2)  }

	\AxiomCm{ \bot \mid \lnot Q(y) \lor S(h(y)) }
	\BinaryInfCm{ Q(x_2) \land P(f(x)) \mid R(x) \lor S(h(x_5))  }

	\BinaryInfCm{
	\big(Q(x_2) \land P(f(x))\big) \lor R(x_0) \mid S(h(x_5))  }

	\BinaryInfCm{ 
	\big(Q(x_2) \land P(f(x))\big) \lor R(x_0) \lor S(h(x_5))  }

\end{prooftree}


$ \forall x_0 \forall x_2 \forall x_5 \big(\lnot Q(x_2) \land P(f(x_0))\big) \lor R(x_0) \lor S(h(x_5))$ 
...
$ f(c, b) \sim y_3$
\end{multicols}
$ (P(\arrowD y_2) \land S(\arrowC x_1)) \lor (\lnot P(\arrowF y_2) \land Q(\arrowA\,\arrowE x_1)) \mid R(\arrowB y_3) $ \drawAB \drawEF \drawCD

$ \forall x_1 \exists y_2 \exists y_3$

\textbf{this is not valid per se as the left hand side only contains $\Sigma$-formulas, but it probably could be fixed by adding some $\Pi$-inferences}
...
\begin{prooftree}
	...
	\noLine\UnaryInfC{}\noLine\UnaryInfC{conjecture: $Q$ and $R$ do not need arrows as they are lifted by the same variable anyway, so constraints on $Q$ do the work}
\end{prooftree}
