...
\begin{lemma} $ \Gamma \entails \lif{(\PI(C) \lor C)} $.
...
\begin{proof}
	...
	\begin{description}
		\item{Resolution.}
			...
			$\Gamma \entails \lif{\PI(C_1) \lor (D\lor l)}$

			$\Gamma \entails \lif{\PI(C_2) \lor (E\lor \lnot l')}$
			...
			\begin{enumerate}
				\item $\Pred(l) \in \Lang(\Gamma) \setminus \Lang(\Delta)$:
					...
					Let $\sigma'$ be as in Lemma \ref{lemma:lif}. $t_1, \ldots, t_n$ must contain all $\Delta$-terms occuring here in formulas and substitutions.
					...
					So by applying $\sigma'$ to $(*)$ and $(\circ)$, we can do a resolution on $\lif{l}\sigma'$ and get
			...
			\color{gray}
					...
					We show that $\Gamma \entails \lif{((\PI(C_1) \land \PI(C_2)) \lor D \lor E)\sigma}$
					By lemma \ref{lemma:lif} with $\sigma'$ as in the lemma,
					$\Gamma \entails \lif{((\PI(C_1) \land \PI(C_2)) \lor D \lor E)} \sigma'$.
