...
\begin{proof}
	...
	\begin{description}
		...
		\item{Paramodulation.}
			...
			$\Gamma \entails \lif{\PI(C_2) \lor (E[r])_\Gamma}$

			By Lemma~\ref{lemma:lift_commute} and Lemma~\ref{lemma:lif}, these imply:

			$\Gamma \entails \lif{\PI(C_1)}\sigma' \lor \lif{D_\Gamma}\sigma' \lor (\lif{s_\Gamma}\sigma') = (\lif{t_\Gamma}\sigma')$

			$\Gamma \entails \lif{\PI(C_2)}\sigma' \lor \lif{(E\occurat{r}{p})_\Gamma}\sigma'$

			$\PI(C) = $

			We show that $\Gamma \entails $
...
$\Gamma \entails \bar Q \lifboth{\PI(\pi)}$;
$\Delta \entails \lnot \bar Q \lifboth{\PI(\pi)}$;
\end{document}
