...
\mytodo{ remove this and do same thing for lemma \ref{lemma:subterm_in_grey_lit} because it's actually required for that }
\begin{lemma}
	...
	\item Suppose that a variable $u$ occurs in $\inv\sigmazmi$ in a maximal colored $\Phi$-term $t$ such that $u\sigma_i$ contains $s$ at a grey position.
		...
		\begin{itemize}
			...
			\item Suppose that $u$ occurs grey in a $\Psi$-literal or in a single-colored $\Psi$-term in $\inv\sigmazmi$.
				...
				\largered{ reconsider the notion of single-colored !}
			...
			\item If none of the above cases apply, then $u$ occurs only grey in $\Phi$-literals and in single-colored $\Phi$-terms in $\inv\sigmazmi$.
				...
				%Note that due to the definition of the unification algorithm, $u\sigma_i$ must occur in $\inv\sigmazmi$.
				%The concrete position of the occurrence is determined by the type of the current inference but immaterial to this reasoning. 
