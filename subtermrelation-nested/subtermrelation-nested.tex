...
\begin{proof}
	...
	\begin{description}
		...
		\item{} Paramodulation.
			...
			Note that $\Gamma \entails \lifdeltanovar{s\tau}\neq \lifdeltanovar{t\tau} \land \lifdeltanovar{\LI(C_1)\tau}$ suffices for $\Gamma \entails \lifdeltanovar{\LIpre(C)}$ and $\Gamma \entails \lifdeltanovar{D_\Gamma\tau}$ implies that $\Gamma \entails \lifdeltanovar{C_\Gamma}$.
			Therefore we obtain that 
			$\Gamma \entails \lifdeltanovar{\LIpre(C)} \lor \lifdeltanovar{C_\Gamma}$.

			Now suppose to the contrary that for a model $M$ of $\Gamma$ that $M \entails \lifdeltanovar{s}\nobreak=\lifdeltanovar{t}$.
				...
				\item
					...
					\begin{itemize}
						\item Suppose $h\occ{s}$ occurs more than once in $\LI(C_2)\tau \lor E\occatp{s}\tau$.
							...
							and
							$\lifdeltanovar{(E\occatp{t})_\Gamma\tau}$
							differ solely at subterms which we can assume to be equal in the model,
