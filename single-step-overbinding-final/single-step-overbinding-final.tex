...
\begin{lemma}
	...
	$\Gamma$-term $t\occ{x_s}$ contains a $\Delta$-lifting variable $x_s$, then $x_s \apath_{G_C} t\occ{x_s}$.
\end{lemma}
\begin{proof}
		...
		\item{}Resolution.
			...
			\begin{itemize}
				...
				\item Suppose a maximal colored $\Gamma$-term in $C_1$ or $C_2$ contains a variable $u$ such that $s$ occurs grey in $u\sigma$. 
					...
					Note that if $s$ occurs grey in $C$, then 
					say at $p$,
					...
					Lemma~\ref{} furthermore asserts that $u_n$ occurs in a resolved or factorised literal $\lambda$ at $\lambda\at{\bhat u_n}$ such that $\lambda{}'\at{\bhat u_n}$ contains the outermost symbol of the $\Delta$-term $s$, where $\lambda{}'$ is the other resolved or factorised literal.
					As $u_n$ is a colored occurrence and $\lambda\sigma = \lambda{}'\sigma$, $\lambda{}'\at{\bhat u_n}$ is a $\Delta$-term contained in a $\Gamma$-term.
					...
					\mytodo{term containing $u_n$ must be $\Gamma$}
