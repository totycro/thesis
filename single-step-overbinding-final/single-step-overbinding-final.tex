...
\begin{defi}
	Let $\Gamma$ and $\Delta$ be sets of first-order forumulas.
...
\begin{defi}[$\AIcol$]
	...
	\begin{itemize}
		...
		\item[Resolution.]
			...
			If $l$ is a colored literal, then
			$\AIcol(C) \defeq \AIcol(C_1) \cup \AIcol(C_2) \cup \{ \lifboth{l\cl\sigma}\tau, \lifboth{l'\cl\sigma}\tau \}$.
			Otherwise $l$ is a grey literal and 
			$\AIcol(C) \defeq \AIcol(C_1) \cup \AIcol(C_2)$.
...
\begin{defi}
	...
	Note that there exists exactly one for every literal of every clause which is a predecessor of $C$.
	...
	\begin{itemize}
		...
		\item[Resolution.]
			...
			$\arr_1 \defeq \{ (p, q) \mid $ maximal colored term $t$ occurs in $x\sigma$ for some variable $x$, $p$ grey occurrence of $t$ in $C$, $q$ maximal colored term containing colored occurrence of $x$ (where the color of $x$ is different from the color of $t$) in $C_1$ or $C_2 \}$
...
\begin{lemma}
	...
	Then there is a sequence of variables $x_1,\dots, x_n$ with $x_1 = x$ such that $t$ occurs grey in  $x_i \sigma$ for $1\leq i \leq n$ and $x_n$ occurs in a resolved or factorised literal $l$ at $l\at{\bhat x_n}$ such that $l'\at{\bhat x_n}$ contains the outermost symbol of $t$, where $l'$ is the other resolved or factorised literal.
\end{lemma}
\begin{proof}
	...
	As the $\mgu$ algorithm produces a unifer which modifies $x_i$, $x_i$ must occur in a resolved or factorised literal, say in $l$ at $l\at{\bhat x_i}$, such that at the other resolved or factorised literal $l'$,  $l'\at{\bhat x_i}$ is an abstraction of a term containing $t$ which is different from $x_i$.
	We distinguish two cases:
	\begin{itemize}
			\item Suppose that $l'\at{\bhat x_i}$ contains the outermost symbol of $t$.
				Then $x_i = x_n$ is the last element in the sequence.

			\item Otherwise $l'\at{\bhat x_i}$ contains a variable $v$ such that $t$ occurs grey in $v\sigma$.
				Then let $x_{i+1} = v$.
				\qedhere
	\end{itemize}
...
\begin{lemma}
	...
	$\Gamma$-term $t\occ{x_s}$ contains a $\Delta$-lifting variable $x_s$, then $x_? \apath_{G_C} t\occ{x_s}$.
\end{lemma}
\begin{proof}
	...
	\begin{description}
		\item{}Base case.
			For $C \in \Gamma\cup\Delta$, no mixed-colored term occurs in $C$ and hence also not in $\lifdeltanovar{C} = \AIclde(C)$.
			As $\AImatde(C)$ is either $\top$ or $\bot$, this situation does not arise here as well.

		\item{}Resolution.
			Suppose the clause $C$ is the result of a resolution step $\inference$ of $C_1: D \lor l$ and $C_2: E \lor \lnot l'$ with $\sigma = \mgu(\inference)$ and $\tau = \aiu(\inference)$.
			There are two possible cases in which a $\Delta$-lifting variable $x_s$ can be subterm of a $\Gamma$-colored term $t\occ{x_s}$ in $\AImatde(C)\lor \AIclde(C)$:

			\begin{itemize}

				\item Suppose a maximal colored $\Gamma$-term in $C_1$ or $C_2$ contains a variable $u$ such that $s$ occurs grey in $u\sigma$. 

					By Lemma~\ref{}, there is a sequence of variable $u_1, \dots, u_n$ such that $u_1 = u$ such that $s$ occurs grey in $u_i \sigma$ for $1\leq i \leq n$. 
					Note that if any variable $u_i$ occurs grey in $C_1$ or $C_2$, say at $p$,
					then $\arr_1$ of Definition~\ref{def:arrows} contains $(p, q)$ and $C\at{q}$ is $t\occ{x_s}$.
					As $\arr_1 \subseteq G_C$, clearly $x_s \apath_{G_C} t\occ{x_s}$.

					Therefore suppose that $u_1, \dots, u_n$ occur only colored in $C_1$ and $C_2$.
					Lemma~\ref{} furthermore asserts that $u_n$ occurs in a resolved or factorised literal $l^*$ at $l^*\at{\bhat u_n}$ such that $l^*{}'\at{\bhat u_n}$ contains the outermost symbol of the $\Delta$-term $s$, where $l^*{}'$ is the other resolved or factorised literal.
					As $u_n$ is a colored occurrence and $l^*\sigma = l^*{}'\sigma$, $l^*{}'\at{\bhat u_n}$ is a $\Delta$-term contained in a $\Gamma$-term.
					So by the induction hypothesis, 
					$x_? \apath_{G_{C_1}} r$, where $r$ is the maximal colored term containing $l^*{}'\at{\bhat u_n}$.
					This however implies that $x_?$ occurs grey in $\AImatde(C)$, $\AIclde(C)$ or $\AIcolde(C)$.
					So by $arr_2$ ???

					\mytodo{must be $\Gamma$}
				...
				\item Suppose a variable $u$ occurs in $C_1$ or $C_2$ such that $u\sigma$ is a multi-colored $\Delta$-term.
					\mytodo{}
			\end{itemize}
