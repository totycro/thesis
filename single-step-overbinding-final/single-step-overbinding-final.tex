...
\begin{proof}
	...
	By Lemma~\ref{lemma:no_colored_terms}, $\AImatde(\pi) \lor \AIclde(\pi)$ does not contain $\Delta$-colored symbols.

	Let $x_1, \dots x_n$ be the $\Delta$-lifting variables of $\AImatde(\pi) \lor \AIclde(\pi)$ and $y_1, \dots y_m$ be the $\Gamma$-lifting variables of $\lifgamma{\AImatde(\pi) \lor \AIclde(\pi)}$.
	Let \[I = \forall x_1 \dots \forall x_n \exists y_1 \dots \exists y_m \lifgamma{\AImatde(\pi) \lor \AIclde(\pi)}.\]
...
graph as annotation; else as in outline below
