...
\begin{proof}
	...
	\begin{description}
		\item{} Suppose term containing max colored term which is $\Delta$-term is introduced into $\Gamma$-colored term.
			...
			existence 2:
			Otherwise $u$ only occurs colored in $C_i$.
			As it is affected by $\sigma$, it must occur in the resolved literal, say at $l\at{\bhat u}$. $l'\at{\bhat u}$ is an abstraction of $u\sigma$ different from $u$.
			If $l'\at{\bhat u}$ contains a $\Delta$-term $\Ra$ ind hyp. 
