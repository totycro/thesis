...
\begin{clemma}
	Let $C$ be a clause in a resolution refutation of $\Gamma\cup\Delta$ and
	$\overline x$ be the $\Delta$-lifting variables occurring in $\AImatde(C) \lor \AIclde(C)$.
...
\begin{clemma}
	Let $\pi$ be a refutation of $\Gamma\cup\Delta$ and $\bhat \pi$ be $\pi$ with $\bhat \Gamma = \Delta$ and $\bhat \Delta = \Gamma$. Then $\Q(\pi)\AImat(\pi) \semiff \lnot \Q(\bhat \pi)\AImat(\bhat \pi)$.
\end{clemma}
\begin{proof}
	...
	We show by induction that $\AImat(C) \semiff \lnot \AImat(\bhat C)$.
...
\end{thm}
\begin{proof}
	\mytodo{still need $\Delta \entails \lnot Q(c) ( \AImat(C) \lor \AIcl(C) )$}
