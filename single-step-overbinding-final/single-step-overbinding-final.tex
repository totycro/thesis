...
{ More elaborate but unneeded approach:
...
\mytodo{move this to definition of resolution}
...
\begin{defi}
	...
	Factorisation: $\Lambda(C) \defeq \{ \lifboth{ \varphi \sigma} \tau \mid \varphi \in \Lambda(C_1) \}$ \comm{here, we could merge the factorised literals if we merge their arrows, but this is not necessary }

	~

	\comm{An alternate definition would be to collect all colored literal (like $\AIcol$) and all of the ones in the interpolant (like $\AImat$), then define $\Lambda(C) \defeq \Lambda_{\operatorname{col}} \cup \Lambda_{\operatorname{mat}} \cup C$.
	}
...
\begin{defi}
	...
	%Hence given $C$, $l.tp$ is a well-defined position and 
	...
	\begin{itemize}
		...
		\item[Resolution.]
			...
			$\arr_1 \defeq \{ (p, q) \mid $ maximal colored term $t$ occurs in $x\sigma$ for some variable $x$, $p$ grey occurrence of $z_t$ in $\AIany(C)$, $q$ maximal colored term containing colored occurrence of $x$, where the color of $x$ is different from the color of $t$ in $\bbar C$ \}\todo{this works in $\AIde$, possibly not in $\AI$}
...
\begin{lemma}
	...
	%Let $\Lambda = \bigcup_{j=1}^{|\bbar C|} \Lambda(C_j)$.
...
\begin{proof}
	...
	\begin{description}
		...
		\item{} Induction step.
			...
			We show first that the property holds for newly introduced lifting variables, and second that the property is not violated in the course of the derivation.

			\begin{itemize}
				\item
					Suppose a $\Gamma$-lifting variable $y_t$ is added to $\AIany(C)$ in $\inference$.
					...
					Then from the inference in the derivation where $y_t$ has been created to $\inference$, at least one variable contained in $t$, say $u$, has been substituted.
				...
				\item 
					...
					Suppose that there exist ancestors of $\bhat y_s$ and $\bdot y_s$ in $\AIany(D)$ for $D\in\bbar C$ which are occurrences of $y_s$.
					\mytodo{ lifting variables may also be the same if they are in different clauses if they do not contain variables }
					Then by the induction hypothesis, $\AIanyde(D)\at{\bhat y_s} = \AIanyde(D)\at{\bdot y_s}$.
					$\bhat y_s$ and $\bdot y_s$ can only be modified by $\aiu(\inference)$, but this affects both of them the same way.
					...
					Suppose that otherwise at both ancestors of $\bhat y_s$ and $\bdot y_s$ are lifting variables but at least one ancestor is $y_r$ such that $r\neq s$.
					Then it is modified by $\aiu(\inference)$ and hence occurs in a resolved or factorised literal $\lambda$, say at $p$.
					However $\aiu(\inference)$ modifies it such $\lambda\at{p} = s$.

					- at this position in $\AIanyde(C)$ also in resolved/factorised literal, so reset

					- if other $y_s$ has been there before: then argue as above, no var changed

					- if other $y_s$ also just created: then also everything in $\AIany(C)$ AS WELL AS $\AIanyde(C)$ is reset.
					\qedhere
...
\begin{lemma}
	...
	Let $C$ be a clause in a resolution refutation of $\Gamma\cup\Delta$.
	If a $\Delta$-lifting variable occurs multiple times in $\AIanyde(C)$ at a grey position, then the terms at the corresponding positions in $\AIany(C))$ are equal.
\end{lemma}
\begin{proof}
	Note that the corresponding terms in $\AIany(C)$ do not exist if the $\Delta$-lifting variable in $\AIanyde(C)$ is contained in a $\Gamma$-term, so we do not consider these.

	initially: just lifted all the same in both.

	step:

	same before, only changed by $\tau$ $\Ra$ same afterwards

	just introduced: then just introduced for both the same

	unified by $\tau$ $\Ra$ either both unified, or one is still exactly the right term (as no vars changed)
	\qedhere
...
\begin{proof}
	...
	But as $\AIcl(\square) = \square$, this simplifies to
