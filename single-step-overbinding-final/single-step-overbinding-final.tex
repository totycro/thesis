...
\mytodo{make sure that lemma 24 isn't misused: in lemma 25, terms can occur in any colored terms, check how that works out for 24}


\begin{lemma}
	\label{lemma:y_sigma_contains_grey_x}
...
\begin{lemma}
	...
	Let $\Phi$ and $\Psi$ be the two distinct colors.
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item
			...
			Then by Lemma~\ref{lemma:y_sigma_contains_grey_x} gives the result.
...
\begin{proof}
	...
	\begin{description}
		...
		\item{}Resolution.
			...
			\begin{itemize}
				\item Suppose a single-colored $\Phi$-term $s\occ{x}$ in $C_1$ or $C_2$ contains a grey occurrence of $x$ and 
					furthermore a single-colored $\Psi$-term $y\occ{z}$ in $C_1$ or $C_2$ contains a variable $z$ such that $x$ occurs grey in $z\sigma$.
