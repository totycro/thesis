...
\begin{lemma}
	Let $\pi$ be a resolution refutation of $\Gamma \cup \Sigma$ and $C_1$ and $C_2$ be the clauses used in a resolution or factorisation step with $\sigma = \mgu(l, l')$, where $l$ and $l'$ are the resolved or factorised literals.
...
\begin{proof}
	...
	Suppose that $x$ does not occur grey in $(C_1 \cup C_2)\sigmaz{i-1}$ as otherwise we are done.
	We show that if a variable $x$ occurs in a single-colored $\Phi$-term in $(C_1 \cup C_2)\sigmazi$, then (1) it does so in $(C_1 \cup C_2)\sigmaz{i-1}$ or (2) there is a color-changing variable $y$ in $(C_1 \cup C_2)\sigmaz{i-1}$ such that $x$ occurs grey in $y\sigma_i$.
	Consider the situations which produce a single-colored $\Phi$-term containing a variable:
	\begin{itemize}
		\item Suppose single-colored $\Phi$-colored term containing $x$ is present in $(C_1 \cup C_2)\sigmaz{i-1}$. Then it is as well in $(C_1 \cup C_2)\sigmaz{i}$.
		\item Suppose that a variable $y$ occurs a single-colored $\Phi$-term in $(C_1 \cup C_2)\sigmaz{i-1}$ such that $x$ occurs grey in $y\sigma_i$.
			Suppose furthermore that $x$ does not occur in a single-colored $\Phi$-term in $(C_1 \cup C_2)\sigmaz{i-1}$ as otherwise we are done.
			As by assumption it does not occur grey in $(C_1 \cup C_2)\sigmaz{i-1}$, $x$ only occurs in single-colored $\Psi$-terms in $(C_1 \cup C_2)\sigmaz{i-1}$.
			But as $x$ occurs grey in $y\sigma_i$, there must be an occurrence $\bhat y$ of $y$ in say w.l.o.g.\ $l\sigmaz{i-1}$ such that $l'\sigmaz{i-1}\at{\bhat y}$ is a subterm where $x$ occurs grey.
			But as $l'\sigmaz{i-1}\at{\bhat y}$ is contained in a single-colored $\Psi$-term, so is $l\sigmaz{i-1}$, so $y$ is a color-changing variable in $(C_1 \cup C_2)\sigmaz{i-1}$. 
	...
	Suppose now that $x$ occurs in $(C_1 \cup C_2)\sigmazi$ in a single-colored $\Phi$-term as well as in a single-colored $\Psi$-term.
	If this is the case in $(C_1 \cup C_2)\sigmaz{i-1}$, then by the induction hypothesis, $x$ occurs grey in $(C_1 \cup C_2)\sigmaz{i-1}$ and consequently also in $(C_1 \cup C_2)\sigmazi$.

	If otherwise $x$ does not occur in a single-colored $\Phi$- or $\Psi$-term in $(C_1 \cup C_2)\sigmaz{i-1}$, then by the reasoning given above, there is a color-changing variable $y$ in $(C_1 \cup C_2)\sigmaz{i-1}$ such that $x$ occurs grey in $y\sigma_i$. 
	By the induction hypothesis, then $y$ occurs grey in $(C_1 \cup C_2)\sigmaz{i-1}$, which directly implies that $x$ occurs grey in $(C_1 \cup C_2)\sigmazi$.
\end{proof}


\begin{lemma}
 	Let $C$ be a clause in a resolution refutation of $\Gamma \cup \Delta$.
	If in $\AImatde(C) \lor \AIclde(C)$	a $\Gamma$-term $t\occatp{x_s}$ contains a $\Delta$-lifting variable $x_s$, then $x_s$ occurs grey in $\AIanyde(C)$.
\end{lemma}
\begin{proof}
	...
	We continue by induction over the construction steps of $\sigma = \sigma_1 \cdots \sigma_n$ and consider the situations under which 
