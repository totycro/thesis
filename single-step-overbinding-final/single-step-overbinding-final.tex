		...
		\item{Resolution.}
			...
			\begin{itemize}
				\item
					...
					If $\tau$ is trivial on $\lifboth{t\cl\sigma}$, we are done as then $\lifboth{t\cl\sigma}\tau = \lifboth{t\sigma}$.
...
\end{lemma}
\mytodo{check if thought to outsource in previous lemma has anything to do with this}
\begin{proof}
	As 
	...
	$l\cl \simgeq \lifboth{l}$ and
	$l'\cl \simgeq \lifboth{l'}$. 
	Furthermore note that the $\simgeq$-relation guarantees that pairs of predicates and terms in this relatiion are equal up to the index of their lifting variables.
...
\begin{proof}
	...
	\begin{description}
		...
		\item{Factorisation.}
			...
			\mytodo{ continue after literal equal lemma has been formalised }
...
\end{thm}
\begin{proof}
	\todo[inline]{This needs to many things I don't yet know how to make precise, so let's start with $\Gamma \entails \dots$}
\end{proof}
