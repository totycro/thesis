...
\begin{defi}
	...
	\begin{itemize}
		...
		\item[Resolution.]
			...
			$\arr_1 \defeq \{ (p, q) \mid $ maximal $\Phi$-term $t$ occurs in $x\sigma$ for some variable $x$, $p$ grey occurrence of $t$ in $C$, $q$ $\Psi$-colored occurrence of $x$ in $C_1$ or $C_2$, $(\Phi, \Psi) \in \{(\Gamma, \Delta), (\Delta, \Gamma)\} \} $ 
...
\begin{lemma}
	...
	$\Gamma$-term $t\occ{x_s}$ contains a $\Delta$-lifting variable $x_s$, then $x_? \apath t\occ{x_s}$
\end{lemma}
\begin{proof}
	...
	\begin{description}
		...
		\item{}Resolution.
			Suppose the clause $C$ is the result of a resolution step $\inference$ of $C_1: D \lor l$ and $C_2: E \lor \lnot l'$ with $\sigma = \mgu(\inference)$ and $\tau = \aiu(\inference)$.
			There are two possible cases in which a $\Delta$-lifting variable $x_s$ can be subterm of a $\Gamma$-colored term $t\occ{x_s}$ in $C$:
			\begin{itemize}
				\item Suppose a $\Gamma$-colored term in $C_1$ or $C_2$ contains a variable $u$ such that $s$ occurs grey in $u\sigma$. 
