...
\begin{defi}[Color alternation $\ca$]
	...
	\begin{cases}
		...
		\max(\ca_\Phi(t_1), \dots, \ca_\Phi(t_n)) & \text{if $t = f(t_1, \dots, t_n)$ is grey} \\
...
\begin{corr}
	...
	If there is a grey literal $\lambda$ or an equality $s=t$ in $\PI(C) \lor C$ for a clause $C$ of a resolution refutation $\pi$,
...
\begin{proof}
	...
	First, suppose that $\inference$ is a resolution or factorisation inference.
