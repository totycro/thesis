...
%\usepackage{color} % for gray comments
%\usepackage{alltt}
...
In a literal or term $\phi$ containing a subterm $t$, $t$ is said to occur \defiemph{below} a $\Phi$-symbol $s$ if in the syntax tree representation of $\phi$, there is a node labeled $s$ on the path from the root to $t$. Note that the colored symbol may also be the predicate symbol.
Moreover, $t$ is said to occur \defiemph{directly below} the $\Phi$-symbol $s$ if it occurs below the $\Phi$-symbol $s$ and in the syntax tree representation of $\phi$ on the path from $s$ to $t$, no nodes with labels with colored symbol occur.
...
\mytodo{ transportierfähig machen; $\PI^*$ entfernen wo geht }
\section{Upper bound}
...
%Let $T_i$ be the set of colored terms with $i$ color alternations.
