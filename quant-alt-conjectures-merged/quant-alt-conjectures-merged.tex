...
%	\begin{itemize}
%		\item
%			Suppose that $s$ is not lifted in this step.
%			The lifting and quantification of $t$ adds one quantifier to $\LIpre(C)$.
%			By the induction hypothesis, as $s$ is contained in $\LI(C)$ and hence by construction of $\LIpre$ in $\LIpre(C')$ for every immediate successor $C'$ of $C$, $n-1$ quantifier alternations are added to $\LIpre(C')$.
%
%		\item
%			Suppose that $s$ is lifted in this step.
%			The lifting and quantification of $t$ adds one quantifier to $\LIpre(C)$.
%			As $s$ is a subterm of $t$, $s$ is lifted and quantified only later than $t$ and the quantifier for $t$ is hence in the scope of the quantifier for $s$.
%	\end{itemize}

\largered{conclude by explaining two main results ; describe how to apply result (term $t$ occs in ??)} 
