...
\begin{lemma}[corresponds to lemma 25 in -final]
	...
	Suppose a s.c.\ $\Phi$-term $s\occ{y}$ occs in $l \sigmaz{i-1}$ or $l'\sigmaz{i-1}$ where $1\varleq i \varleq n$ and $\sigma_0 = \id$ s.t.\ $\dom(\sigma_i) = y$ and a var $x$ occurs grey in $y\sigma_i$.
	At least one of the following statments holds:

	\begin{enumerate}
		\item $x$ occurs grey in $l\sigmazi$
		\item var $z$ s.t.\ $x$ occs grey in $z\sigmazi$ OR the var x itself occur in s.c.\ $\Phi$-term in  $l\sigmaz{i-1}$  
...
\end{lemma}
\begin{proof}
	At some point, say $i$, the is diff pair $(y, t)$ is resolved, where $t$ is an abstraction of $\epsilonx$, say $t = \epsilonz$ 

	So $\sigma_i = \{ y \mapsto \epsilonz \}$, where either $z=x$ or $x$ occurs grey in $z\sigma$ (in total: $\epsilonz\sigma = \epsilonx$).

	If $y$ occs grey, we are done as this then is a grey occ of $x$ in $l\sigma$. (1)

	If $y$ occs in a s.c.\ $\Psi$-term, then $y$ is a col change var, where $x$ then is a col change var in $l\sigma$. (3)

	So supp it only occurs in $\Phi$-terms (note that clauses aren't var disjoint anymore here in general).

	$y$ occs at $\bhat y$ in $\lambda$ and $\lambda'\at{\bhat y}$ is exactly $\epsilonz$.
	But  $\lambda\at{\bhat y}$ and  $\lambda'\at{\bhat y}$ share the prefix, so $\epsilonz$ is 
	contained in a $\Phi$-term, and in $l\sigma$, that's a grey occurrence of $x$ (this occurrence of $\epsilonz$ exists at least in stage $i-1$, but possibly not in $l$). (2) \mytodo{}
\end{proof}
...
\begin{lemma}[corresponds to lemma 26 in -final]
	...
	Suppose a variable $y$ occs in  $l \sigma_{0\ra i}$ or $l'\sigma_{0\mapsto i}$ where $0\varleq i \varleq n$ and $\sigma_0 = \id$
	s.t.\ $x$ occurs in a s.c.\ $\Phi$-term in $y\sigma$.
	At least one of the following statments holds:

	\begin{enumerate}
		\item $x$ occurs grey in $l\sigma$
		\item $x$ occurs in a s.c.\ $\Phi$-term in ????
			\mytodo{}
		\item there is a col change where $x$ is the col change var in $l\sigma$ \mytodo{}
	\end{enumerate}
\end{lemma}
...
\item there is a color change with $y$ as color change var \mytodo{where? $\sigmarange{0}{i}$?}
