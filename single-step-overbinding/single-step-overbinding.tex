...
for a position $p$, $\pos{p_i}$ denotes whatever $p_i$ refers to in its respective clause.
...
$\arr{C}$ for a clause $C$ is a set of ordered pairs or term positions in $C$ or $\PI(C)$.

$\warr{C}$ for a clause $C$ is a set of unordered pairs or term positions in $C$ or $\PI(C)$.
...
\begin{enumerate}
	\item For each initial clause $C$ in $\Gamma \cup \Delta$:
		...
		Add to $\arr{C}$ all $(p_1, p_2)$
		such that $p_1$ contains only grey symbol and $\pos{p_1} = \pos{p_2}$ but $p_1 \neq p_2$.

		Add to $\warr{C}$ all $\{p_1, p_2\}$ such that there is a colored symbol in $p_1$ and a possible different one in $p_2$ and $\pos{p_1} = \pos{p_2}$.
	...
	\item 
		...
		$(p_1, p_2) \in \arr{C} \semiff $ $\poslit{p_i}$ comes from literal $l_i$ in a clause $D$ in an original clause set, $i\in\{1, 2\}$, (both from same $D$), and $(l_1.\posterm{p_1}, l_2.\posterm{p_2}) \in \arr{D}$ 
...
\begin{enumerate}
	...
	\item
		...
		if $l$ and $l'$ don't have the same color:

		$\AImatrix(C) = (\lnot \lifboth{l} \land \AImatrix(C_1)) \lor (\lifboth{l} \land \AImatrix(C_2))$
		...
		$(p_1, p_2) \in \arr{C}$ implies that $u_i < u_j$ if $u_i$ replaces $t_i$ and $u_j$ replaces $t_j$ and $p_1$ points into $t_i$ and $p_2$ points into $t_j$.
...
\begin{proof}
	...
	$\PI(C) = (l \land \PI(C_2)) \lor (\lnot l \land \PI(C_1)) $

	supp $l$ grey:
	$\Gamma \entails \bar Q_n  \lifgamma{ \lifdelta{PI(C_i)\lor C_i} } $ for $i \in \{1,2\}$

	Need:
	$\Gamma \entails \bar Q_n \lifboth{{PI(C)\lor C} }$
	...
	TODO: what if a predicate doesn't make it into the interpolant.

	TODO: use formalised version
