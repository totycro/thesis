...
\newcommand{\lemmOne}{Lemma $\eth$}
\newcommand{\lemmTwo}{Lemma $\pitchfork$}
...
\begin{proof}
	...
	OLD from first principles reasoning (complete this in case \lemmOne{} is flawed):
...
\begin{proof}
	...
	For $C \in \Gamma$, $\AImatrix(C) = \bot$ and $\AIclause = \lifboth{C_\Gamma} = \lifgamma{C}$. By the restriction, $\lifgamma{C} = C$ and $\Gamma \entails C$.\todo{restriction used here}
	...
	\begin{description}
		\item{Resolution.}
			...
			$\Gamma \entails \AImatrix(C_i) \lor \AIclause({C_i}_\Gamma)$, $i\in\{1,2\}$, or expressed differently (cf.~Lemma \ref{lemma:literals_in_aiclause}):

			$\Gamma \stackrel{\markA}\entails \AImatrix(C_1) \lor \Big(\AIclause((C_1)_\Gamma)\setminus\{\lifboth{l_\Gamma}\}\Big) \lor \lifboth{l_\Gamma}$

			$\Gamma \stackrel{\markB}\entails \AImatrix(C_2) \lor \Big(\AIclause((C_2)_\Gamma)\setminus\{\lnot \lifboth{l'_\Gamma}\}\Big) \lor \lnot \lifboth{l'_\Gamma}$

			Let $\sigma'$ as in \lemmOne. Then by \lemmOne{} and \lemmTwo:
			\begin{flalign*}
				\AIclause(C_\Gamma) &=
				\lifboth{\Big( (\AIclause((C_1)_\Gamma) \setminus \{\lifboth{l_\Gamma}\}) \lor (\AIclause((C_2)_\Gamma)\setminus   \{\lifboth{\lnot l'_\Gamma}\}) \Big)\sigma} & \\
																																																																																			 &=
			...
			\end{flalign*}
			Furthermore,
			$\sigma = \mgu(l, l')$, so $\lifboth{l\sigma} = \lifboth{l'\sigma}$ and $\lifboth{l}\sigma' = \lifboth{l'}\sigma'$ by \lemmOne.
			...
			\begin{itemize}
				...
				\item Suppose that $l$ and $l'$ are $\Gamma$-colored.
					...
					$\Gamma \entails \lifboth{\AImatrix(C_1)}\sigma' \lor \lifboth{\Big(\AIclause((C_1)_\Gamma)\setminus\{\lifboth{l}\}\Big)}\sigma' \lor \lifboth{l}\sigma'$

					$\Gamma \entails \lifboth{\AImatrix(C_2)}\sigma' \lor \lifboth{\Big(\AIclause((C_2)_\Gamma)\setminus\{\lifboth{l}\}\Big)}\sigma' \lnot \lifboth{l'}\sigma'$

					As $\lifboth{l}\sigma' = \lifboth{l'}\sigma'$, we can perform a resolution step to arrive at:

					$\Gamma \entails \lifboth{\AImatrix(C_1)}\sigma' \spa\lor \lifboth{\Big(\AIclause((C_1)_\Gamma)\setminus\{\lifboth{l}\}\Big)}\sigma' \spa\lor
					\lifboth{\AImatrix(C_2)}\sigma' \spa\lor \lifboth{\Big(\AIclause((C_2)_\Gamma)\setminus\{\lifboth{l}\}\Big)}\sigma' $
			...
			$\Gamma \entails \AImatrix(C_1) \lor \Big(\AIclause((C_1)_\Gamma)\setminus\{\lifboth{l_\Gamma}\}\Big) $

			$\Gamma \entails \AImatrix(C_2) \lor \Big(\AIclause((C_2)_\Gamma)\setminus\{\lnot \lifboth{l'_\Gamma}\}\Big)$

			But this implies that:

			$\Gamma \entails \Big(\AImatrix(C_1)\spa\land \AImatrix(C_2)\Big) \lor 
			\Big(\AIclause((C_1)_\Gamma)\setminus\{\lifboth{l_\Gamma}\}\Big) \lor
			\Big(\AIclause((C_2)_\Gamma)\setminus\{\lnot \lifboth{l'_\Gamma}\}\Big)$
