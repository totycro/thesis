...
\begin{frame}{Craig Interpolation (1/2)}
	...
	\vfill
	\begin{theorem}[\cite{Craig57linear}]

		Let \myA{}
		and \myB{} be finite sets of first-order formulas 
		%where
		%\begin{itemize}
		...
		\end{itemize}
		Then there is a interpolant $\mygray I$ such that: 
		\begin{itemize}
	...
	\begin{example}
		\begin{itemize}
			\item
				...
				and
				%$\Delta = \{ \forall x (\mygray P(x) \limpl \colB Q(x)), \exists y \colB Q(y) \}$.
...
\begin{frame}{Applications and Motivation}
	...
	\begin{block}{Motivation}
		\begin{itemize}
			\item Little attention for interpolation in first-order logic with equality 
...
\subsection{}
\begin{frame}{Interpolant extraction (\cite{Huang95})}
	\small
	%Approach:
	%\begin{enumerate}
