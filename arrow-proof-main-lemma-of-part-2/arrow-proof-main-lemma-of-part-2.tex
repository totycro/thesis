...
\begin{clemma}(this is actually what the procedure should establish. Make sure to have some kind of network of all colored occurrences of a variable.)\nopagebreak 
	\label{lemma:colored_merge_ai_de}
	\begin{itemize}
		\item
			If some colored occurrence of $x$ in $s\occ{x}$ in $\AIde(C)$ is unified with a term $t$ such that for a variable $y$, it holds that $y\apath t$, then add an arrow from every grey occurrence of $y$ in $C$ to $s\occ{x}$.
	\comm{for the upper lemmas}
		\item 
			If some colored occurrence of $x$ in $s\occ{x}$ in $\AIde(C)$ is unified with a term $t$ such that for a lifting variable $x_i$, it holds that $x_i \apath t$, then add an arrow from every grey occurrence of $x_i$ in $C$ to $s\occ{x}$.
		...
		\item[Suppose for some $\gamma_{j\bm '}\occ{u}$ in (w.l.o.g.) $\AIde(C_1)$ that a maximal colored term] which is a $\Delta$-term occurs in $u\sigma$.
			...
			\begin{itemize}
				...
				\item
					...
					\begin{itemize}
						\item Suppose that $z_i$ occurs grey in $C_2$.
							\begin{itemize}
						...
						\item Suppose that $z_i$ does not occur grey in $C_2$ but in $C$ (if it does not occur in $C$, there is nothing to prove).
							Hence there is a variable $z$ in $C_i$ such that $z_i$ occurs grey in $\lifdeltanovar{z\sigma}\tau$.
							\mytodo{ICI ICI ICI}
					\end{itemize}
...
\begin{conj}
	...
	\textbf{\emph{possible fix: add grey occurrence of each variable in some place and apply everything to these and consider them when doing the ordering }}}
\end{conj}
\begin{proof}
