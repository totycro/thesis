...
\mytodo{CHECK UP ON THIS PROOF, then tackle next two.}

\mytodo{Fix up major proof below with the three new lemmas}


\begin{clemma}
	Let $x$ be a variable in $\AIclausede(C)$ which has a grey occurrence and a colored occurrence $\bhat x$.
	Then  there is an path from a term containing $x$ to a term containing $\bhat x$ using arrows from $\arr(C)$.
	...
	\begin{description}
		\item[Suppose $x$ is introduced into a colored term $t$ by means of unification.]
			...
			\begin{itemize}
					...
					\item Suppose $\haty$ occurs in a maximal colored term, say $s\occ{y}$.
						...
						\begin{itemize}
							...
							\item 
								Suppose $\hatyPrime$ does not contain $x$.
			...
			\item [Suppose a term containing $t\occ{x}$ with $t$ colored is in $\ran(\sigma)$.]
				...
				figure:
				$C_1: Q(\dots y \dots) \lor l\occ{\bhat y}\quad$
