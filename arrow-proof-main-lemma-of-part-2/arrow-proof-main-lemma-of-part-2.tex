\documentclass[,%fontsize=12pt,%
	paper=a4,% 
	%landscape,
	DIV9, % mehr text pro seite als defaultyyp
	%DIV10, 
	%DIV=calc,%
	twoside=false,%
	liststotoc,
...
\mytodo{for rewriting, make sure to handle coloring correctly (lifting vars are not colored) and make sure to be clear on whether we talk about $C$, $\AI(C)$ or $\AIde(C)$.}
				...
				\item Otherwise there are only colored occurrences of $y$, so also $l\at{\bhat y}$ is a colored occurrence. Let it be contained in the maximal colored term $s\occ{y}$.
					...
					\begin{itemize}
						\item Suppose that $x$ occurs grey in $C_2$. Then by the induction hypothesis, $x\apath l'\at{\bhat y}$ and so
							...
							As all other occurrences of $y$ are contained in colored terms and $x$ occurs in $y\sigma$, there is a unifier-induced arrow such that $x\apath y$. \NB{don't need ind hyp anymore here}
...
\begin{lemma}
	...
	\comm{possibly not really needed as same var is always lifted by same lifting var, they can never diverge syntactically. Still this is used in the proof.}
	\NB{possibly get rid of it by interpreting arrows as from term to term, not from occurrence to occurrence. This way, there is an implicit connection between every grey occurrence of a variable}
\end{lemma}
\begin{proof}
	Induction start: by definition.
...
\begin{lemma}(this is actually what the procedure should establish. Make sure to have some kind of network of all colored occurrences of a variable.)\nopagebreak 
	...
	\NB{it seems to be more useful to just write ``unifier arrow 1/2''}
	...
	\comm{for the upper lemmas}
		\item 
			...
			\comm{for $\AIde$ lemma (probably needs to be applied to $\AI^\Gamma$ separately)}
	\end{itemize}

	\NB{\normalfont intuition of difference: grey variable changes are handled elsewhere}
...
\begin{proof}
	...
	\begin{description}
		\item[Suppose for some $\gamma_{j\bm '}\occ{u}$ in (w.l.o.g.) $\AIde(C_1)$ that a maximal colored term] which is a $\Delta$-term occurs in $u\sigma$.
			...
			\NB{
					By unifier-induced arrows 2, we have that $z_i \apath \gamma_{j\bm '}\occ{u}$ straight away in case there is a grey occurrence of $z_i$. The induction hypothesis seems to make sure that it actually exists.
				}
			...
			\begin{itemize}
				...
				\item
					...
					\todo{only mention of "no $\Delta$-terms" here, looks somewhat generalisable}
	...
	\item[Suppose for some variable $u$ in (w.l.o.g.) $\AIde(C_1)$ that] $u\sigma$ contains a mixed-colored $\Gamma$-term containing $z_i$.
			...
			\NB{unifier-induced arrows seem to replace all this as in the other case}
			...
			\begin{itemize}
				...
				\item Suppose $l'\at{\bhat u}$ does not contain a mixed-colored term.
					...
					So this shows that $z_i \apath l'\at{\bhat u}$. \mytodo{recheck when remark is made more formal, but it probably can't imply less than the other case does.}
			\end{itemize}
			...
			\begin{itemize}
				...
				%\item Suppose $\bdot u$ is a colored occurrence and $\bhat u$ is a colored occurrence.
					\qedhere
			\end{itemize}
		...
		\item[Suppose for some $\Gamma$-term $\oldgam\occ{u}$ in $\AI\de(C_1)$ or $\AI\de(C_2)$, $u\sigma$ contains a $\Delta$-term.]\hfill\nopagebreak
			...
			\begin{itemize}
				...
				\item Suppose $\hatu$ occurs in a maximal colored term which is a $\Delta$-term.
					...
					\largered{no $\Delta$-terms in $\AIde(C)$.}
					\begin{prooftree}
