			...
			~
				...
				\begin{itemize}
					\item Suppose $l'\at{\bhat y}$ contains $x$.
						...
						Otherwise figure: $ y \lor z \lor l[f(y), g(z)]\quad\quad \lnot l[f(h(x)), g(x)]$

						Otherwise $x$ occurs only colored in $C_2$. 
						If $x$ occurs grey in $C$, then it is derived from a grey occurrence of a variable $z$ in $C_1$ (such that $x$ occurs grey in $z\sigma$), and $x$ occurs colored in $l'\at{\bhat x}$ and $z$ occurs colored in $l\at{\bhat x}$ (with the same prefix).
						...
						By Lemma~\ref{lemma:merge_arrow_between_same_colored_terms}, $l'\at{\bhat x} \mpath l'\at{\bhat y}$, so $x\apath l\at{\bhat y}$ in $C$.
