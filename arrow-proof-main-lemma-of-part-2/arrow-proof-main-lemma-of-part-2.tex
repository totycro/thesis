...
\begin{clemma}(this is actually what the procedure should establish. Make sure to have some kind of network of all colored occurrences of a variable.)\nopagebreak 
	...
	\begin{itemize}
		\item
			If $x$ also occurs in the resolved literal and is unified with a term $t$ of the other resolved literal such that for a variable $y$, it holds that $y\apath t$, then add an arrow from every grey occurrence of $y$ in $C$ to $s\occ{x}$.
	\comm{for the upper lemmas}
		\item 
			If $x$ also occurs in the resolved literal and is unified with a term $t$ of the other resolved literal 
...
\begin{proof}
	...
	\begin{description}
		...
		\item[Suppose for some variable $u$ in (w.l.o.g.) $\AIde(C_1)$ that $u\sigma$ contains a mixed-colored $\Gamma$-term.]
