...
\begin{proof}
	...
	\begin{description}
		...
		\item[Suppose for some variable $v$ in $\AI\de(C_1)$ or $\AI\de(C_2)$, $v\sigma = \newgam\occ{\delta_i}$ for some $i$.]\hfill
			...
			At one point in the unification algorithm, $v$ is substituted by an abstraction of $\newgam\occ{\delta_i}$.
			Let $p$ be the position of the occurrence of $v$ in $l$ which leads to this situation.
			Furthermore, let $p'$ be the position corresponding to $p$ in $l'$.

			Note that any arrow from or to $p'$ also applies to $p$ in $\arrFinal(C)$ and hence to $\newgam\occ{x_i}$ as they are merged due to occurring in the resolved literal.
