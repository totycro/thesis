...
In the extension of the ``Interpolation Algorithm'' to include paramodulation inferences in \cite[p.~183]{Huang95}, this notion is used to distinguish between the respective cases.
Translated into our notation in the context of our corresponding Definition~\ref{def:PI_paramod}, the conditions for the three cases are:
\begin{enumerate}
...
We consider the following example, which illustrates that the definition of the conditions for the cases above is to be read as ``maximal colored term, which is $\Phi$-colored'' (or more concisely: ``maximal colored $\Phi$-term'') in place of ``maximal $\Phi$-term''.
...
\begin{exa}
	...
	Note that at the paramodulation inference \markC{}, case~\ref{case_1} is selected due to $d$ occurring in the maximal $\Gamma$-colored term $g(d)$, 
	...
	By combining these two derivation by means of a resolution inference on the last remaining literal employing a trivial substitution, we obtain the interpolant $\PI(\square)$:
				$
