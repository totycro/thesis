...
In \cite[Definition 3]{Huang95}, a maximal occurrence of a $\Gamma$ ($\Delta$)-term is defined to be an occurrence of a $\Gamma$ ($\Delta$) term which is not subterm of a larger $\Gamma$ ($\Delta$)-term.

In the extension of the ``Interpolation Algorithm'' to include paramodulation inferences\cite[p.~183]{Huang95}, this term is used to distinguish between the respective cases.
Translated into our notation in the context of our definition in \ref{def:PI_paramod}, the conditions for the three cases are:
\begin{itemize}
...
Note that when reading this definition in the strict sense, an ambiguity arises:
It is very well possible for a term to be a subterm of a maximal $\Gamma$-term and a maximal $\Delta$-term at the same time.

We consider the following example, which illustrates that the definition of the conditions for the cases above must be read as ``maximal colored term, which is $\Phi$-colored'' (or more concisely: ``maximal colored $\Phi$-term) in place of ``maximal $\Phi$-term''.

\begin{exa}
	Let 
	$\Gamma = \{ P(x) \lor \lnot Q(x), \lnot P(y) \lor Q(y), c=d, \lnot R(g(d)), \lnot S(g(c))  \}$
	and
	$\Delta = \{ S(v) \lor \lnot Q(h(v)), R(u) \lor  Q(h(u)) \}$.
	%\Gamma = \{ P(x) \lor \lnot Q(x), c=d, \lnot R(g(d)), \lnot P(v)  \}$
	%and $\Delta = \{ R(u) \lor  Q(h(u)) \}$.

	We present a resolution refutation of $\Gamma \cup \Delta$ in combination with the interpolant extraction such that each label is of the form $C\mid \PI(C)$, where $C$ is the clause of the refutation.
	For the sake of brevity, we sometimes write simplified but logically equivalent versions of $\PI(C)$.
	%\begin{landscape}

	Note that at the paramodluation inference \markC{}, the maximal $\Gamma$-colored term $g(d)$ is picked to be added to be interpolant, even though it is contained in the maximal $\Delta$-colored term $h(g(d))$.

	{
		...
		%\begin{adjustwidth}{-2.5em}{0em}
			...
			\begin{prooftree}
				...
				\RightLabelm{\lkrule{\resruleres}{u\mapsto g(d)}}
				%\RightLabelm{\resruleres}
				\BinaryInfCm{ \lnot R(g(d)) \mid  Q(h(g(d))) }

				\AxiomCm{{P(x) \lor \lnot Q(x)}}

				\RightLabelm{\lkrule{\resruleres}{x \mapsto h(g(d))}}
				\BinaryInfCm{ {\lnot R(g(d)) \land \lnot Q(h(g(d)))}\mid P(h(g(d))) }

				\AxiomCm{{c=d}}
				\RightLabelm{\lkruleremark{\resrulepar}{\id}{\markC}}
				\BinaryInfCm{ (c=d \land \lnot R(g(d)) \land \lnot Q(h(g(d)))) \lor (c\neq d \land g(c) = g(d))   \mid P(h(g(c))) }
			...
			\begin{prooftree}
				...
				\AxiomCm{{ \lnot S(g(c))}}
				\AxiomCm{{  S(v) \lor \lnot Q(h(v)) }} 

				\RightLabelm{\lkrule{\resruleres}{v \mapsto g(c)}}
				\BinaryInfCm{ \lnot S(g(c)) \mid \lnot Q(h(g(c))) }

				\AxiomCm{{\lnot P(y) \lor Q(y)}}

				\RightLabelm{\lkrule{\resruleres}{y \mapsto h(g(c))}}
				\BinaryInfCm{ {\lnot S(g(c)) \land Q(h(g(c)))} \mid \lnot P(h(g(c))) }
	...
	By combining these two derivation by means of a resolution inference on the last remaining literal with a trivial substitution, we obtain the interpolant $\PI(\square)$:

	\noindent
				$
				(c=d \land \lnot R(g(d)) \land \lnot Q(h(g(d)))) \spam\lor (c\neq d \land g(c) = g(d))  \spam\lor
				{\lnot S(g(c)) \land Q(h(g(c)))} $

				By lifting $\PI(\square)$ and adding appropriate quantifiers, we obtain the final interpolant $I$: 
			...
			\]\[
				...
				Note that as $\Gamma \entails c=d$, no model of $\Gamma$ can satisfy $(c\neq d \land y_{g(c)} = y_{g(d)})$.
				The remaining disjuncts imply that 
				$\forall x_{h(g(c))} \forall x_{h(g(d))} ( \lnot Q(x_{h(g(d))}) \lor Q(x_{h(g(c))}))$,
				but we can easily find a model of $\Gamma$ where at least one domain element does not satisfy the predicate $Q$ and another domain element does satisfy it.
				Any such model is a countermodel.

				LOOKS BAD:
				\[
					\exists y_c \exists y_d \exists y_{g(c)} \exists y_{g(d)} \forall x_{h(g(c))} \forall x_{h(g(d))} 
				\]
				\[
					\Big(
				(y_c=y_d \land \lnot R(y_{g(d)}) \land \lnot Q(x_{h(g(d))})) \spam\lor
			\]\[
					(y_c\neq y_d \land y_{g(c)} = y_{g(d)})  \spam\lor
					{\lnot S(x_{g(c)}) \land Q(x_{h(g(c))})}
					\Big)
				\]


				\begin{comment} FAILED
					{\small
						...
						\begin{adjustwidth}{-4em}{0em}
							...
							\begin{prooftree}
								...
								\AxiomCm{{c=d} \mid \bot}
								\RightLabelm{\lkrule{\resrulepar^{\markC}}{\id}}
								\BinaryInfCm{ (c=d \land \lnot R(g(d)) \land \lnot Q(h(g(d)))) \lor (c\neq d \land g(c) = g(d))   \mid P(h(g(c))) }


								\AxiomCm{\lnot P(v) \mid \bot }

								\RightLabelm{\lkrule{\resruleres}{v\mapsto h(g(c))}}
				\BinaryInfCm{ (c=d \land \lnot R(g(d)) \land \lnot Q(h(g(d)))) \lor (c\neq d \land g(c) = g(d))   \mid \square}
