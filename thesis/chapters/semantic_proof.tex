...
\begin{proof}
	...
	\begin{enumerate}[I.]
		\item
			...
			Since these cases are analogous, we only consider the first. 
			...
			But as we have just witnessed that $T_{j}$ and $T_{j+1}$ must be different, $T_{j} \cup \{ \mathcal{A}_{j} \}$ and $T'_{j}$ must be inseparable.
			...
			Hence \ref{theory_construction_2a} must be the case. Therefore $A_j$ is of the form $\exists x A$ and $T_{j+1} = T_{j} \cup \{ \mathcal{A}_{j}, A\subst{x/c_i} \}$.
			As $T_{j} \cup \{ \mathcal{A}_{j}, A\subst{x/c_i}  \}$ and $T'_j$ are separabe, there exists a formula $B$ in the language $\Lang(T_{j} \cup \{ \mathcal{A}_{j}, A\subst{x/c_i}  \}) \cap \Lang(T'_j)$
			such that $ T_{j} \cup \{ \mathcal{A}_{j}, A\subst{x/c_i} \} \entails B$ and $T'_j \entails \lnot B$.
			Since $c_i$ is a fresh variable and therefore is not contained in $\Lang(T'_j)$, $c_i$ does not occur in $B$.
			However $\Lang(T_{j} \cup \{ \mathcal{A}_{j}, A\subst{x/c_i}  \}) = \Lang(T_{j} \cup \{ \mathcal{A}_{j}\}) \cup \{ c_i \}$

			\largered{ICI}
				
			But then as $T_{j} \cup \{ \mathcal{A}_{j} \}$ and $T'_j$ are inseparable, there can not be such a formula $B$.
