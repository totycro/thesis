...
\begin{defi}
	...
	Then $\lift{\Phi}{\phi}{x}$ denotes $\phi\abstraction{t_1/x_1}\ldots\abstraction{t_n/x_n}$.
\end{defi}
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item[Paramodulation.] Suppose the last rule application is an instance of paramodulation. Then it is of the form:
			\begin{prooftree}
				\AxiomCm{C_1: D \lor s=t}
				\AxiomCm{C_2: E\occur{s}}
				\BinaryInfCm{C: D \lor E\occur{t}}
			\end{prooftree}
			...
			$\Gamma \entails \lif{\PI(C_2) \lor E\occur{s}_\Gamma}$.

			By Lemma \ref{lemma:lift_commute}, we get that 

			$\Gamma \entails \lif{\PI(C_1)} \lor \lif{D_\Gamma} \lor \lif{s}=\lif{t}$ and 

			$\Gamma \entails \lif{\PI(C_2)} \lor \lif{E\occur{s}_\Gamma}$.

			We distinguish two cases:\nopagebreak
			\begin{enumerate}
				\item Suppose $s$ does not occur in a maximal $\Delta$-term $h\occur{s}$ in $E\occur{s}$ which occurs more than once in $\PI(E(s)) \lor E\occur{s}$.

					We show that $\Gamma \entails \lif{ (s=t \land \PI(C_2)) \lor (s\neq t \land \PI(C_1)) \lor (D \lor E(t))_\Gamma}$, which subsumes the cases \ref{def:PI_paramod_2} and \ref{def:PI_paramod_3} of 
					Definition \ref{def:PI}. By Lemma \ref{lemma:lift_commute}, this is equivalent to

					$\Gamma \entails (\lif{s}=\lif{t} \land \lif{\PI(C_2)}) \lor (\lif s\neq \lif t \land \lif{\PI(C_1)}) \lor (\lif{D_\Gamma} \lor \lif{E(t)_\Gamma})$

					Suppose that in a model $M$ of $\Gamma$ 
					$M \cancel\entails \lift{\Delta}{ D_\Gamma }{x}$ and 
					$M \cancel\entails \lift{\Delta}{ E(t)_\Gamma }{x}$.
					We show that then, depending on whether $\lif{s} = \lif{t}$ holds in $M$, one of the first disjuncts holds in $M$.

					Then in case $M \entails \lif{s} = \lif{t}$ we also get
					$M \cancel\entails \lift{\Delta}{ E(s)_\Gamma }{x}$ and consequently by the induction hypothesis $M\entails \lif{\PI(C_2)}$.
				...
				\item Otherwise $s$ occurs in a maximal $\Delta$-term $h\occur{s}$ in $E\occur{s}$ which occurs more than once in $\PI(E(s)) \lor E\occur{s}$.

					Then models are possible in which $s=t$ and therefore $\lif{s}=\lif{t}$ holds, while at the same time $\lif{h\occur{s}} \liff \lif{h\occur{t}}$ does not as $h\occur{s}$ and $h\occur{t}$ are replaced by distinct variables due to being different $\Delta$-terms.

					Therefore we amend the proof of case \ref{njktahjtkhltah} as follows:

					In the case $M \entails \lif{s} = \lif{t}$, 

					TODO: what if multiple $h(s)$? which one?


					However as here $\PI(C)$ contains the with respect to case \ref{njktahjtkhltah} additional disjunct $s=t \land h\occur{s} \neq h\occur{t}$, 


					TODO: argue why not at other case
