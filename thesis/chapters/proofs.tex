...
\begin{defi}[Propositional interpolant extraction.]
	...
	\label{def:huang_orig_PI}
	\begin{itemize}
		\item[Base case.]
			If $C \in \Gamma$, $\PI(C) = \bot$. 
			If otherwise $C \in \Delta$, $\PI(C) = \top$. 
		\item[Resolution.]
			\label{def:huang_orig_PI_resolution}
		...
		\item[Paramodulation.]
	\label{def:huang_orig_PI_paramod}
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item[Resolution.]
			...
			We consider the respective cases from definition \ref{def:huang_orig_PI_resolution}:
		...
		\item[Paramodulation.]	
			...
			First, we show that $\PI(C)$ as constructed in case \ref{def:huang_orig_PI_paramod_3} of the definition is a propositional interpolant in any of these cases:
			...
			This concludes the argumentation for case \ref{def:huang_orig_PI_paramod_3}. 

			The interpolant for case \ref{def:huang_orig_PI_paramod_1} differs only by an additional formula added via a disjunction and hence condition \ref{huang_orig_rel_prop_interpol_cond1} of definition \ref{def:huang_orig_rel_prop_interpol} holds by the above reasoning.
			...
			The situation in case \ref{def:huang_orig_PI_paramod_2} is somewhat symmetric: 
			As a tautology is added to the interpolant with respect to case \ref{def:huang_orig_PI_paramod_1}, condition \ref{huang_orig_rel_prop_interpol_cond1} is satisfied by the above reasoning.
			For condition \ref{huang_orig_rel_prop_interpol_cond2}, consider that the negated interpolant for case \ref{def:huang_orig_PI_paramod_1} implies the negated interpolant for this case.
...
that the algorithm stated in \ref{def:huang_orig_PI} gives closely related results for both $\pi$ and its corresponding propositional refutation.
...
\cbstart
\todo[inline]{this part occurs in other proof}
...
\begin{defi}[Lifting]
	...
	$\varphi$ a formula or a term, $Z=\{\colterm{1}, \dotsc, \colterm{n}\}$ the maximal $\Phi$-terms for $\Phi \in \{\Gamma, \Delta\}$ in $\varphi$ and $z_1, \ldots, z_n$ fresh variables, referred to as \defiemph{lifting variables}.

	Then $\lft{\Phi}{z}{\varphi}$ denotes 
	$\lft{\Phi}{z}{\varphi, Z}$ which is defined as follows:
\[\lft{\Phi}{z}{\varphi, Z} \defeq \begin{cases} \varphi & Z = \emptyset \\ \lft{\Phi}{z}{\varphi/{\colterm{i}/z_i}, Z\setminus\{\colterm{i}\}} & \text{$\colterm{i} \in Z$ such that $\colterm{i}$ is not subterm of another term in $Z$} \end{cases}\]
	\largered{deprecated to other version in huang proof without $P_P$}
	We also denote $\varphi$ lifted of both $\Gamma$- and $\Delta$-terms by $\lifboth{\varphi}$ if the variables replacing the colored terms are clear from the context or are not crucial.
\end{defi}
...
\begin{lemma} 
  \label{lemma:lift_commute_huang}
...
\begin{lemma}
	...
	\label{lemma:naming_of_colored_variablesHuang}

	\largered{not quite right with $\expa$, see proof without $P_P$}
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item[Resolution.] Suppose the last rule application is an instance of resolution. Then it is of the form:
			...
			which by Lemma \ref{lemma:lift_commute_huang} is equivalent to
			...
			\begin{enumerate}
				\item Suppose $l$ is $\Gamma$-colored.
					...
					Several applications of Lemma \ref{lemma:lift_commute_huang} give
				...
				\item Suppose $l$ is $\Delta$-colored.
					...
					$M \notentails \lift{\Delta}{ E_\Gamma }{x}$, it follows that $M \entails \lift{\Delta}{ \PI(C_1) }{x}$ and $M \entails \lift{\Delta}{ \PI(C_2) }{x}$. Hence by Lemma~\ref{lemma:lift_commute_huang}
				...
				\item Suppose $l$ is grey.
					...
					and several applications of Lemma \ref{lemma:lift_commute_huang} give
		...
		\item[Factorisation.] Suppose the last rule application is an instance of factorisation. Then it is of the form:
			...
			By Lemma \ref{lemma:lift_commute_huang}, 
			...
			so by again applying Lemma \ref{lemma:lift_commute_huang}, we arrive at
		...
		\item[Paramodulation.] Suppose the last rule application is an instance of paramodulation. Then it is of the form:
			...
			By Lemma \ref{lemma:lift_commute_huang}, we get that 
			...
			\begin{enumerate}
				\item Suppose $s$ does not occur in a maximal $\Delta$-term $h\occur{s}$ in $E\occurat{s}{p}$ which occurs more than once in $\PI(E(s)) \lor E\occurat{s}{p}$.
					...
					We show that $\Gamma \entails \lif{ (s=t \land \PI(C_2)) \lor (s\neq t \land \PI(C_1)) \lor (D \lor E\occurat{t}{p})_\Gamma}$, which subsumes the cases \ref{def:huang_orig_PI_paramod_2} and \ref{def:huang_orig_PI_paramod_3} of 
					Definition \ref{def:huang_orig_PI_paramod}. By Lemma \ref{lemma:lift_commute_huang}, this is equivalent to
				...
				This reflects case \ref{def:huang_orig_PI_paramod_1} of Definition \ref{def:huang_orig_PI_paramod}.
...
The interpolant extraction procedure $\PI$ exhibits a convenient property which is termed \emph{symmetry} in \cite[Definition 5]{interpolantStrenth} and can be stated formally as follows:

\newcommand{\primexHuang}[1]{\ensuremath{\bhat{#1}}}
\begin{lemma}
	\label{lemma:symmetryHuang}
	Let $\pi$ be a resolution refutation of $\Gamma \cup \Delta$ and
	$\primexHuang\pi$ be $\pi$ with $\primexHuang\Gamma = \Delta$ and $\primexHuang\Delta = \Gamma$.
	Then $\PI(\pi) \semiff \lnot \PI(\primexHuang\pi)$.
\end{lemma}
\begin{proof}
	We prove this lemma by induction on $\pi$.
	Let $\primexHuang\varphi$ denote the clause/formula/literal/term in $\primexHuang\pi$ corresponding to the clause/formula/literal/term $\varphi$ in $\pi$.

	\newcommand{\p}[1]{\primexHuang{#1}}
	\begin{itemize}
		\item[Base case.]
		...
		\item[Resolution.]
			...
			\begin{enumerate}
					\item $l$ is $\Gamma$-colored. Then $\primexHuang l$ is $\Delta$-colored.
						\begin{align*}
							\PI(C) &= \PI(C_1) \lor \PI(C_2)\\
														 &\semiff \lnot ( \lnot \PI(C_1) \land \lnot \PI(C_2) )\\
														 &= \lnot ( \PI(\primexHuang C_1) \land \PI(\primexHuang C_2) ) \\
														 &= \lnot \PI(\primexHuang C)  
						\end{align*}
					\item $l$ is $\Delta$-colored. This case can be argued analogously.

					\item $l$ is grey. Then $\primexHuang l$ is grey.\nopagebreak 
			\begin{align*}
				\PI(C) 
		...
		\item[Paramodulation.]
			...
			\begin{enumerate}
				\item $r$ occurs in a maximal $\Delta$-term $h\occur{r}$ in $E\occur{r}$ and $h\occur{r}$ occurs more than once in $E\occur{r} \lor \PI(E\occur{r})$.
					Then $\primexHuang r$ occurs in a maximal $\Gamma$-term $\primexHuang h\occur{r}$ in $\primexHuang E\occur{r}$ and $\primexHuang h\occur{r}$ occurs more than once in $\primexHuang E\occur{r} \lor \PI(\primexHuang E\occur{r})$.
					\begin{align*}
						\PI(C) &= [ ( s=t \land \PI(C_2) ) \lor (s\neq t \land \PI(C_1)) ]\sigma \lor (s=t \land h\occur{s} \neq h\occur{t})\sigma\\
						 &= [ ( s=t \land \lnot \PI(\primexHuang C_2) ) \lor (s\neq t \land \lnot \PI(\primexHuang C_1)) ]\sigma \lor (s=t \land h\occur{s} \neq h\occur{t})\sigma\\
						 &\semiff \lnot [ ( s\neq t \lor \PI(\primexHuang C_2) ) \land (s= t \lor \PI(\primexHuang C_1)) ]\sigma \land \lnot (s\neq t \lor h\occur{s} = h\occur{t})\sigma\\
						 &\semiff \lnot [ ( s = t \land \PI(\primexHuang C_2) ) \lor (s\neq t \land \PI(\primexHuang C_1)) ]\sigma \land \lnot (s\neq t \lor h\occur{s} = h\occur{t})\sigma\\
						 &= \lnot \PI(\primexHuang C)
					\end{align*}

				\item $r$ occurs in a maximal $\Gamma$-term $h(r)$ in $E\occur{r}$ and $h(r)$ occurs more than once in $E\occur{r} \lor \PI(E\occur{r})$.
					This case can be argued analogously.
				\item Otherwise:
					\begin{align*}
						\PI(C) &= [ ( s=t \land \PI(C_2) ) \lor (s\neq t \land \PI(C_1)) ]\sigma \\
									 &= [ ( s=t \land \lnot \PI(\primexHuang C_2) ) \lor (s\neq t \land \lnot \PI(\primexHuang C_1)) ]\sigma \\
									 &\semiff \lnot [ ( s\neq t \lor \PI(\primexHuang C_2) ) \land (s=t \lor \PI(\primexHuang C_1)) ]\sigma \\
									 &\semiff \lnot [ ( s=t \land \PI(\primexHuang C_2) ) \lor (s\neq t \land \PI(\primexHuang C_1)) ]\sigma \\
									 &= \lnot \PI(\primexHuang C)
					\qedhere
					\end{align*}
			\end{enumerate}
...
This lemma can be leveraged to show a counterpart of Lemma \ref{lemma:gamma_entails_lifted_interpolantHuang} for $\Delta$:\nopagebreak

\begin{corr}
	\label{cor:delta_entails_lifted_interpolantHuang}
	Let $\pi$ be a resolution refutation of $\Gamma \cup \Delta$. 
	Then $\Delta \entails \lft{\Gamma}{x}{ \lnot \PI(C) \lor C }$ for $C$ in $\pi$.
\end{corr}
\begin{proof}
	Build $\primexHuang \pi$ from $\pi$ using $\primexHuang \Gamma = \Delta$ and $\primexHuang \Delta = \Gamma$ as initial clause set partition.
	\newline
	By Lemma \ref{lemma:gamma_entails_lifted_interpolantHuang}, $\primexHuang \Gamma \entails \lft{\primexHuang\Delta}{x}{ \PI(\primexHuang C) \lor \primexHuang C }$ for $\primexHuang C$ in $\primexHuang \pi$.
	\newline
	By Lemma \ref{lemma:symmetryHuang},
$\primexHuang \Gamma \entails \lft{\primexHuang\Delta}{x}{ \lnot \PI(C) \lor \primexHuang C }$ for the clause $C$ in $\pi$ corresponding to $\primexHuang C$ in $\primexHuang \pi$. 
...
\end{thm}
\begin{proof}
	By Lemma \ref{lemma:gamma_entails_lifted_interpolantHuang}, $\Gamma \entails \forall x_1 \ldots \forall x_m\,\lifdelta{\PI(\pi)}$ where $m$ is the number of maximal $\Delta$-colored terms in $\PI(\pi)$.
	...
	By Corollary \ref{cor:delta_entails_lifted_interpolantHuang} $\Delta \entails \forall y_1 \dots \forall y_m\,\lnot \lift{\Gamma}{\PI(\pi)}{y}$, where $m$ is the number of $\Gamma$-colored terms in $\PI(\pi)$.
	...
	By Lemma \ref{lemma:naming_of_colored_variablesHuang} and as all variables which replace colored terms are bound, 
