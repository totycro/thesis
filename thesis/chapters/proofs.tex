	...
	\begin{itemize}
		...
		\item[Resolution.]
			...
			\begin{enumerate}
				...
				\item If $l$ is grey: $\PI(C) = [(l \land \PI(C_2)) \lor (l' \land \PI(C_1)) ]\sigma $
			\end{enumerate}
...
\begin{lemma}
	Let $\pi$ be a resolution refutation of $\Gamma \cup \Delta$ and $I = \PI(\square)$ be the propositional interpolant of the empty clause derived in $\pi$.
...
\begin{thm}
	Let $\pi$ be a resolution refutation of $\Gamma \cup \Delta$ and $I = \PI(\square)$ the propositional interpolant of the empty clause derived in $\pi$.
	Furthermore, let $z_1, \dots, z_n$ be the variables which replace the colored terms in $\lifgamma{\lifdelta{I}}$ ordered by their length.
	Then
	$Q_1 z_1 \ldots Q_n z_n\,\lifgamma{\lifdelta{I}}$, where $Q_i$ is $\forall$ ($\exists$) if $z_i$ replaces a $\Delta$ ($\Gamma$)-term, is an interpolant.
\end{thm}
\begin{proof}
	By Lemma \ref{lemma:gamma_entails_lifted_interpolant}, $\Gamma \entails \forall x_1 \ldots \forall x_m\,\lifdelta{I}$ where $m$ is the number of maximal $\Delta$-colored terms in $I$.

	A term in $\lifdelta{I}$ is either $x_i$, $1 \leq i \leq m$, a grey term or a $\Gamma$-terms.
	Let $t$ be a maximal $\Gamma$-term in $\lifdelta{I}$ and $x_{j_1}, \dots, x_{j_k}$ the variables replacing $\Delta$-terms in $t$.
	...
	In $\lifgamma{\lifdelta{I}}$, $t$ is replaced by some $z_j$, which is existentially quantified.
	...
	Therefore $\Gamma \entails Q_1 z_1 \ldots Q_n z_n\,\lifgamma{\lifdelta{I}}$.

	By magic, $\Delta \entails \forall y_1 \dots \forall y_m\,\lnot \lift{\Gamma}{I}{y}$, where $m$ is the number of $\Gamma$-colored terms in $I$.
	...
	$\Delta \entails\nolinebreak{} \overline Q_1 z_1 \dots \overline Q_n z_n\,\lnot \lft{\Delta}{x}{\lft{\Gamma}{y}{I}}$ where $\overline Q_i = \exists$ ($\forall$) if $Q_i = \forall$ ($\exists$).
	Therefore also
	$\Delta \entails\nolinebreak{} \lnot Q_1 z_1 \dots Q_n z_n\,\lft{\Delta}{x}{\lft{\Gamma}{y}{I}}$.
	By Lemma \ref{lemma:naming_of_colored_variables} and as all variables which replace colored terms are bound, 
	$\Delta \entails\nolinebreak{} \lnot Q_1 z_1 \dots Q_n z_n\,\lft{\Gamma}{y}{\lft{\Delta}{x}{I}}$.

	As it is now easy to see that $Q_1 z_1 \dots Q_n z_n\,\lft{\Gamma}{y}{\lft{\Delta}{x}{I}}$ contains no colored symbol, it is an interpolant.
\end{proof}
