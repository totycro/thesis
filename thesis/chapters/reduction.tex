...
This method is due to Craig (\cite{Craig57three}).
...
\begin{defi}[Translation of formulas including axioms]
	For first-order formulas $A$, let $\TransAll(A) =
	\left(\bigwedge_{B\in\FAX(\Lang(A))} B \right) \land
	\left(\bigwedge_{B\in\EAX(\Lang(A))} B \right) \land
	\Trans(A)$ and for sets of first-order formulas $\Phi$, let $\TransAll(\Phi) = \FAX(\Lang(\Phi)) \cup  \EAX(\Lang(\Phi)) \cup \Trans(\Phi)$.
\end{defi}
...
\begin{proof}
	...
	First, note that as $M \entails \EAX(\LangSym)$, by Proposition \ref{prop:equivalence_relation},  $E^M$ is an equivalence relation.
	Let $D$ be the domain of $M$.
	We build a model $M'$ with the domain 
	$D' = D/{E^M}$, i.e.\ the the congruence relation of $D$ modulo $E^M$.
	The interpretation $\calI'$ of $M'$ is obtained from the interpretation $\calI$ of $M$ by 
	replacing every occurrence of a domain element $d$ by its respective congruence class $\{ x \mid (d, x) \in E^M\}$.\todo{how to write this in a nicer way?}{}
	As $M \entails \EAX(\LangSym)$, it satisfies the congruence axioms with respect to every function and predicate symbol, and therefore $\calI'$ is well-defined.
...
\begin{proof}
	...
	- equality axiom -> paramodulation recht sinnfrei

	- reguläres axiom -> linke seite 

	- schnitt ?!? 

	(beweis kann länger werden, aber wie geht das? inessential cuts bleiben doch über?)
