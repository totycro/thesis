...
We present his proof in a modified form in this section and in more original form in appendix~\ref{sec:huang}.
...
\begin{remark}
	...
	A predicate symbol occurring in a clause in the resolution derivation is $\Phi$-colored if its predecessor is.
	...
	In the definition due to \cite{Huang95}, this is not the case in general.
\end{remark}
...
\newcommand{\colterm}[1]{s_{#1}}
	...
	The \defiemph{lifting} of $\varphi$, denoted by $\lifphi{\varphi}$, is an abbreviation for 
	...
	\[
		...
		\begin{cases}
			...
			\lifphi{\psi, Z\setminus\{\colterm{i}\}} & \parbox[t]{0.6\linewidth}{$\colterm{i} \in Z$ such that $\colterm{i}$ is not subterm of another term in $Z$, $\psi$ is created from $\varphi$ by replacing every occurrence of $\colterm{i}$ by $z_{\expa(\colterm{i})}$} 
		\end{cases}
	\]
	To simplify the syntax, we sometimes write $\lifphinovar{\varphi}$ or $\lifboth{\varphi}$ if the lifting variables or the lifting variables and the color of the terms to lift are clear from the context or not of the essence.
...
we here have that $\lifboth{z_t\sigma} = \lifboth{z_t} = z_t$, but $\lifboth{z_t\tauell} = \lifboth{z_{{t\sigma}}} = z_{{t\sigma}}$
...
\begin{proof}
	...
	\begin{indproof}
		\indproofitem{Resolution}
			...
			The induction hypothesis gives $\Gamma \entails \lifdelta{ I_1 } \lor \lifdelta{ l_\Gamma \lor l'_\Gamma \lor D_\Gamma }$.
			By applying $\tauelldelta$
			and Lemma~\ref{lemma:lifting_subst_commute}, we obtain
			$\Gamma \entails \lifdelta{ I_1\sigma } \lor \lifdelta{ l_\Gamma\sigma \lor l'_\Gamma\sigma \lor D_\Gamma\sigma }$.
			...
			$\Gamma \entails \lifdelta{ I_1\sigma } \lor \lifdelta{ l_\Gamma\sigma \lor D_\Gamma\sigma }$,
			which is nothing else than $\Gamma \entails \PIstep(\inference, \bar I) \lor \lifdelta{C_\Gamma}$.
		...
		\indproofitem{Paramodulation}
			...
			$\lifdelta{r\sigma} = \lifdelta{s\sigma}$.
...
\begin{lemma}
	...
	If $\inference$ is an inference of $\pi$ using the clauses $C_1, \dots, C_n$ and
	$I_1, \dots, I_n$ and 
