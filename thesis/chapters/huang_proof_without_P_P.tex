...
\begin{defi}[Lifting]
	...
	\[
		...
		\begin{cases}
			\varphi & Z = \emptyset \\
			%\lft{\Phi}{z}{\varphi\abstractionOp{\colterm{i}/z_{\colterm{i}}}, Z\setminus\{\colterm{i}\}} & \parbox[t]{0.4\linewidth}{$\colterm{i} \in Z$ such that $\colterm{i}$ is not subterm of another term in $Z$} \\
...
\begin{restatable}[Commutativity of lifting and substitution]{lemma}{lemmaCommutLiftSubst}
	...
	\[
		x \sigma' =
		\begin{cases} 
			z_{t\sigma} & \text{ if } x = z_t \text{ for some $t$ }\\%\text{ and } t\sigma \neq t \\
...
\begin{proof}
	...
	%
	%Suppose that $t\sigma = t$. Then $\lifphi{t}\sigma' = z_t\sigma' = z_{t\sigma}$.
	% this does not happen due to unfold-lift
...
\begin{exa}
	\mytodo{fix notation below: $ C | \PI(C)$}

	Let $\Gamma = \{ G(x, y) \lor L(x, y) \lor x = y, \lnot G(v, f(v)), \lnot P(w) \lor \lnot P(f(w)) \}$
	and 
	$\Delta = \{ P(d), \lnot P(z) \lor \lnot L(z, u) \}$.

	{ \footnotesize

		\begin{prooftree}
			\AxiomCm{ G(x, y) \lor L(x, y) \lor x = y }
			\AxiomCm{ \lnot P(z) \lor \lnot  L(z, u) }
			\BinaryInfCm{  G(x, y) \lor x=y \lor \lnot P(x) }

			\AxiomCm{ \lnot P(w) \lor \lnot P(f(w)) }
			\AxiomCm{ P(d)}
			\BinaryInfCm{ \lnot P(f(d)) }

			\BinaryInfCm{ G(x, f(d)) \lor \lnot P(x) \lor \lnot P(y) }
			\UnaryInfCm{ G(x, f(d)) \lor \lnot P(x)  }

			\AxiomCm{ \lnot G(v, f(v)) }

			\BinaryInfCm{  \lnot P(d)  }
		...
		\begin{prooftree}
			\AxiomCm{ \bot}
			\AxiomCm{ \top }
			\BinaryInfCm{  L(x, y) \markC }

			\AxiomCm{ \bot}
			\AxiomCm{ \top }
			\BinaryInfCm{ \lnot P(d) }

			\BinaryInfCm{ (x = f(d) \land \lnot P(f(d)) ) \lor (x \neq f(d) \land L(x, f(d))) }
			\UnaryInfCm{  (x = f(d) \land \lnot P(f(d)) ) \lor (x \neq f(d) \land L(x, f(d))) }

			\AxiomCm{ \bot }

			\BinaryInfCm{  (d = f(d) \land \lnot P(f(d)) ) \lor (d \neq f(d) \land L(d, f(d))) }

			\AxiomCm{ \top  }

			\BinaryInfCm{ \lnot P(d) \lor  \big( (d = f(d) \land \lnot P(f(d)) ) \lor (d \neq f(d) \land L(d, f(d))) \big)}
		\end{prooftree}

		$ \lnot P(d) \lor  \big( (d = f(d) \land \lnot P(f(d)) ) \lor (d \neq f(d) \land L(d, f(d))) \big)$

		~

		$ \lnot P(d) \lor  (d = f(d) \land \lnot P(f(d)) ) \lor (d \neq f(d) \land L(d, f(d))) $
		...
		$ \forall x_d \exists y_{f(d)} ( \lnot P(x_d) \lor  (x_d = y_{f(d)} \land \lnot P(y_{f(d)}) ) \lor (x_d \neq y_{f(d)} \land L(x_d, y_{f(d)})) )$

		~

		neg:

		$ \exists x_d \forall y_{f(d)} ( P(x_d) \land  (x_d \neq y_{f(d)} \lor P(y_{f(d)}) ) \land (x_d = y_{f(d)} \lor \lnot L(x_d, y_{f(d)})) )$
		...
		\begin{prooftree}
			...
			\AxiomCm{ \lnot P(z) \lor \lnot  L(z, u) \mid \top}
			\BinaryInfCm{  G(x, y) \lor x=y \lor \lnot P(x) \mid (L(x,y) \land \top) \lor (\lnot L(x,y) \land \bot)}
			\noLine
			\UnaryInfCm{  G(x, y) \lor x=y \lor \lnot P(x) \mid L(x,y)}

			\AxiomCm{ \lnot P(w) \lor \lnot P(f(w)) \mid \bot }
			\AxiomCm{ P(d)\mid \top}
			\BinaryInfCm{ \lnot P(f(d)) \mid P(d) \markC  \hl{error}}

			\BinaryInfCm{ G(x, f(d)) \lor \lnot P(x) \lor \lnot P(y) \mid (x=f(d) \land P(d)) \lor (x\neq f(d) \land L(x, f(d))) }
			\UnaryInfCm{ G(x, f(d)) \lor \lnot P(x)  \mid (x=f(d) \land P(d)) \lor (x\neq f(d) \land L(x, f(d))) }

			\AxiomCm{ \lnot G(v, f(v)) \mid \bot}

			\BinaryInfCm{  \lnot P(d)  \mid (d=f(d) \land P(d)) \lor (d\neq f(d) \land L(d, f(d))) \markC }

			\AxiomCm{ P(d) \mid \bot }

			\BinaryInfCm{  \square \mid P(d) \land (  (d=f(d) \land P(d)) \lor (d\neq f(d) \land L(d, f(d))) ) \markC }
