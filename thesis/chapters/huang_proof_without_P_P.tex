...
In order to quantify terms in the propositional interpolant approprately, we need to sort them according to a particular order:
...
\begin{proof}
	...
	Note that $Z^{i+1} = Z^i \cup \{r_{i+1}\}$. 
	...
	However clearly $\lifdelta{ r_{i+1} }$ in general contains $\Delta$-lifting variables. Let $x_s$ be a $\Delta$-lifting variable in $\lifdelta{ r_{i+1} }$.
