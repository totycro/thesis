...
\begin{defi}[Lifting]
	...
	$\varphi$ a formula or a term, $Z=\{\colterm{1}, \dotsc, \colterm{n}\}$ the maximal $\Phi$-terms for $\Phi \in \{\Gamma, \Delta\}$ in $\varphi$ and $z_{\colterm{1}}, \ldots, z_{\colterm{n}}$ fresh variables, referred to as \defiemph{lifting variables}.

	Then $\lft{\Phi}{z}{\varphi}$ denotes % $\phi\abstractionOp{\colterm{1}/z_1}\dots\abstractionOp{\colterm{n}/z_n}$.
...
\mytodo{check out what we actually have/need for equality of $s$ and $t$}
...
\begin{proof} 
	...
	%Suppose that $s$ and $t$ are syntactically equal. Then $h\occur{t}$ and $h\occur{s}$ are lifted by the same variable and $\lifdelta{E\occurat{t}{p}} \semiff{} \lifdelta{E\occurat{s}{p}}$.
	...
	%Then as by Lemma~\ref{lemma:lift_equality} $M\entails s=t$,  $M \entails\nolinebreak s=\nolinebreak t \;\land\;\lifdelta{h\occur{s}}  \neq \lifdelta{h\occur{t}}$ and we are done.
	%Then $M \entails\nolinebreak s=\nolinebreak t \;\land\;\lifdelta{h\occur{s}}  \neq \lifdelta{h\occur{t}}$ and we are done.
	%Otherwise $M\entails \lifdelta{h\occur{s}} = \lifdelta{h\occur{t}}$.
	But then $M \entails \lifdelta{E\occurat{s}{p}} \semiff{} \lifdelta{E\occurat{t}{p}}$.
	%If $s$ and $t$ are syntactically equal, then $\lifdelta{h\occur{s}} = \lifdelta{h\occur{t}}$ and $\lifdelta{E\occurat{s}{p}} \semiff{} \lifdelta{E\occurat{t}{p}}$, which implies that $M\notentails \lifdelta{E\occurat{s}{p}}$.
...
\begin{proof}[Proof of Lemma~\ref{lemma:gamma_entails_interpolant}]
		...
		\item{Resolution.}
			...
			\begin{enumerate}
				...
				\item $l$ is grey. Then $\PI(C) = [(l \land \PI(C_2) ) \lor (\lnot l' \land \PI(C_1))]\sigma$.
					...
					$\Gamma \entails \big(\lifdelta{l}\sigma' \land \lifdelta{\PI(C_2)}\sigma'\big)\spas\lor\allowbreak \big(\lnot \lifdelta{l'}\sigma' \land \lifdelta{\PI(C_2)}\sigma'\big)\spas\lor\allowbreak \lifdelta{D_\Gamma}\sigma' \lor \lifdelta{E_\Gamma}\sigma'$.
			...
			$\Gamma \entails \lifdelta{\PI(C_2)} \spas\lor \lifdelta{(E\occurat{r}{p})_\Gamma}$ respectively.
				...
				\begin{enumerate}
					\item Suppose $s$ does not occur in a maximal $\Delta$-term $h\occur{s}$ in $E\occurat{s}{p}$, which occurs more than once in $\PI(C_2) \lor (E\occurat{r}{p})_\Gamma$.
						...
						\begin{itemize}
							\item
								Suppose that $r$ does not occur in a maximal $\Delta$-term in $(E\occat{r}{p})_\Gamma$.
						...
						So in any of these cases, by \markB{} and by applying $\sigma'$ and Lemma~\ref{lemma:lif}
						...
						\spas\lor \Big(\lifdelta{D_\Gamma\sigma} \lor \lifdelta{(E\occat{t}{p})_\Gamma\sigma}\Big)$, 
					which by Lemma~\ref{lemma:lift_commute} is nothing else than
						$\Gamma \entails \lifdelta{\PI(C) \lor C}$.


						\mytodo{PROOF OF FIRST CASE IS DONE HERE. RECHECK LEMMA 4.7. THEN CONTINUE WITH OTHER CASE}
						...
						Let $M$ be a model of $\Gamma$. Let $M \notentails \lifdelta{D_\Gamma}\sigma' \lor \lifdelta{(E\occurat{t}{p})_\Gamma}\sigma'$ as otherwise we would be done. We show that depending on the truth value of  $\lifdelta{s} = \lifdelta{t}$ in $M$, either the first or second disjunct of the above formula holds.

						Suppose that $M \entails \lifdelta{s} \neq \lifdelta{t}$. 
						Then by~\markA, $M \entails \lifdelta{\PI(C_1)}$ and hence $M \entails \lifdelta{\PI(C_1)}\sigma'$.

						On the other hand, suppose that $M \entails \lifdelta{s} = \lifdelta{t}$.
						Then by Lemma~\ref{aga5tg5ba}, as $s$ at $p$ in $E\occurat{s}{p}$ does not occur in a maximal $\Delta$-term, 
						$M \notentails \lifdelta{E\occurat{s}{p}}$.
						Hence also $M \notentails \lifdelta{E\occurat{s}{p}}\sigma'$ and
						by Lemma \ref{lemma:lif}, $M\notentails \lifdelta{(E\occurat{s}{p})\sigma}$. 

						Due to $\sigma=\mgu(s, r)$, $s\sigma$ and $r\sigma$ are syntactically equal.
						Suppose they are both not $\Delta$-colored.
						Then the lifting does not affect them and 
						$\lifdelta{(E\occurat{s}{p})\sigma}$ and $\lifdelta{(E\occurat{r}{p})\sigma}$ are the same formula.
				Otherwise the lifting will replace them with the same variable and we as well get that
				$\lifdelta{(E\occurat{s}{p})\sigma}$ and $\lifdelta{(E\occurat{r}{p})\sigma}$ are the same formula.

				By Lemma $\ref{lemma:lif}$, 
				$\lifdelta{(E\occurat{s}{p})}\sigma' = \lifdelta{(E\occurat{r}{p})}\sigma'$,
				so from $M \notentails \lifdelta{E\occurat{s}{p}}\sigma'$, it follows that $M \notentails \lifdelta{(E\occurat{r}{p})}\sigma'$

				Then by~\markB{}, we arrive at $M\entails \lifdelta{\PI(C_2)}\sigma'$


				%The following two lemmas show that $M \notentails \lifdelta{E\occurat{r}{p}}\sigma'$, which by~\markB{} implies that $M\entails \lifdelta{\PI(C_2)}\sigma'$, or $M \entails s=t \;\land\;\lifdelta{h\occur{s}} \neq \lifdelta{h\occur{t}}$.

				%\begin{lemma} $\sigma=\mgu(s, r)$ and $M\notentails \lifdelta{E\occurat{s}{p}}\sigma'$ imply that $M\notentails \lifdelta{E\occurat{r}{p}}\sigma'$.  \end{lemma}\nopagebreak 
