...
\begin{proof}[Proof of Lemma~\ref{lemma:gamma_entails_interpolant}]
	...
	\begin{description}
		...
		\item{Factorisation.}
			...
			As $\sigma = \mgu(l, l')$, $l\sigma$ and $l'\sigma$ are syntactically equal, hence $\lifdelta{l\sigma} = \lifdelta{l'\sigma}$.%\todo[noline,size=\tiny]{syntactically equal? does ``equal'' suffice?. see also $s\sigma=r\sigma$ below}
...
\label{klehjy}
				...
				Let $M$ be a model of $\Gamma$. Let $M \notentails \lifdelta{D_\Gamma}\sigma' \lor \lifdelta{(E\occurat{t}{p})_\Gamma}\sigma'$ as otherwise we would be done. We show that depending on the truth value of  $\lifdelta{s} = \lifdelta{t}$ in $M$, either the first or second conjunct of the above formula holds.
