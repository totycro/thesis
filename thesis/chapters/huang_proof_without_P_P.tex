...
\begin{defi}[Lifting]
	...
	$\varphi$ a formula or a term, $Z=\{\colterm{1}, \dotsc, \colterm{n}\}$ the maximal $\Phi$-terms for $\Phi \in \{\Gamma, \Delta\}$ in $\varphi$ and $z_{\colterm{1}}, \ldots, z_{\colterm{n}}$ fresh variables, referred to as \defiemph{$\Phi$-lifting variables} or \defiemph{lifting variables} if the coloring is clear from the context.
...
\begin{proof}
	...
	Item~\ref{ths6hrh} can be argued analogously as $\pi$ is also a refutation of $\bhat \Gamma \cup \bhat \Delta$, where $\bhat\Gamma = \Delta$ and $\bhat\Delta = \Gamma$.\todo[inline]{this is wrong, see lemma 4.10 and 4.11 (it used to say $\Delta \entails \PI(\pi)$}
