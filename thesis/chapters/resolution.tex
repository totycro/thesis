...
\begin{comment}
	\begin{lemma}
		Let $M$ be a model in the language $\Lang(\delta(A))$ for a formula $A$ without existential quantifiers.
		Then $M \entails A$ if and only if $M \entails \delta(A)$.
	\end{lemma}
	\begin{proof}
...
\begin{proof}
	...
	In the case where $M_A' \notentails A$, we know that $M_A' \notentails L_A$, so $D_A$ holds as well.\todo{use $M_A' \notentails A$ or $M_A' \entails \lnot A$}
...
\begin{proof}
	...
	\begin{itemize}
		...
		%(L_A \lor L_G) \land (L_A \lor \lnot L_H) \land (\lnot L_A \lor \lnot L_G \lor L_H) 
		%\forall \bar x \big(L_A(\bar x) \lor \lnot L_H(\bar z)\big) \land \forall \bar x \big(\lnot L_A(\bar x) \lor \lnot L_G(\bar y) \lor L_H(\bar z)\big)  
		...
		We know that $M\notentails G$ or $M \entails H$.

		By the other lemma item for $n-1$, $M \notentails \delta(G)$
...
\begin{proof}
	...
	%Suppose that $\delta(\Phi) \entails A$ and $M \entails \Phi$.
	%Suppose that $\Phi \entails A$ and $M \entails \delta(\Phi)$.
\end{proof}
