...
This usally makes use of intermediate normal forms which are defined as follows:

\begin{defi}
	A formula is in \defiemph{Negation Normal Form (NNF)} if negations appear only directly in front of atoms.
	A formula is in \defiemph{Conjunctive Normal Form (CNF)} if it is a conjunction of disjunctions of literals.
\end{defi}

In this context, the conjuncts of a formula in CNF are interpreted as clauses.
...
Step \ref{step_nnf_trans} can be achieved by solely pushing the negation inwards in order to receive an equivalent formula. This clearly has no effect on the interpolants.
Step \ref{step_skolem_trans} and \ref{step_cnf_trans} on the other hand do not produce equivalent formulas as they introduce new symbols.
...
\begin{proof}
	...
	none of them are containd in $L(\sk(\Gamma)) \intersect L(\sk(\Delta))$.
...
\begin{defi}
For every occurrence of a subformula $B$ of $A$, we introduce a new atom $L_B$. 
For each of them, we create a defining clause:
...
\begin{itemize}
	...
	$D_B: (\lnot B \lor L_B) \land (B \lor \lnot L_B)  $
	\item[If $B$ is $\lnot G$:]~

	$D_B: (L_B \lor L_G) \land (\lnot L_B \lor \lnot L_G)  $
	\item[If $B$ is $G \land H$:]~

		$D_B: (\lnot L_B \lor L_G) \land (\lnot L_B \lor L_H) \land (L_B \lor \lnot L_G \lor \lnot L_H)  $
	\item[If $B$ is $G \lor H$:]~

		$D_B: (L_B \lor \lnot L_G) \land (L_B \lor \lnot L_H) \land (\lnot L_B \lor L_G \lor L_H)  $
	\item[If $B$ is $G \limpl H$:]~

		$D_B: (L_B \lor L_G) \land (L_B \lor \lnot L_H) \land (\lnot L_B \lor \lnot L_G \lor L_H)  $
	\item[If $B$ is $\forall x G$:]~

		$D_B: \forall x (\lnot L_B \lor L_G) \land \forall x(L_B \lor \lnot L_G)$
\end{itemize}
...
\end{prop}
\begin{proof}
	As $\sk(\Gamma)$ and $\sk(\Delta)$ share no occurrence of a subformula, the set of new atoms which are introduced in $\delta(\sk(\Gamma))$ and $\delta(\sk(\Delta))$ respectively are disjoint.
