...
\begin{defi}[Incrementally lifted interpolant]
	...
	\begin{description}
		...
		\item{} Induction step.
			If $C$ is the result of an inference $\inference$ using the clauses $\bar C$, then $\LIpre(C) \defeq \PIstep(\inference, \LIpre(C_1), \dots, \LIpre(C_n))$.
...
\begin{exa}
	...
	\mytodo{more text, use figure to make clear that it's a separate page?}
