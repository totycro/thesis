...
\begin{lemma}
	...
	Then if a term $t$ occurs in $\LIpre(C)$ or $\LI(C)$, no subterm $s$ of $t$ is lifted.
\end{lemma}
\begin{proof}
	...
	Now suppose that the lemma holds for the clauses $C_1, \dots, C_n$ which are used in an inference $\inference$ to deduce the clause $C$ using the unifier $\sigma = \mgu(\inference)$.
...
\end{lemma}
\begin{proof}
	Let $T = \{t_1, \dots, t_n\}$ be the set of maximal colored terms in $\LIpre(C)$ for which some lifting conditions applies in ascending subterm order.
	We use $Q_i z_{t_i}$ to denote either $\exists y_{t_i}$ in case $t_i$ is $\Gamma$-colored or $\forall x_{t_i}$ in case $t_i$ is $\Delta$-colored.
	Furthermore, we denote by $\lifpart^*(\LIpre(C), T_i)$ the result of lifting all terms of $T_i$ and replacing the lifting variables by fresh ones analogous to step~\ref{lemma_part_renaming} of the lifting procedure of $\LI$.

	We show the result by an induction over \[\Gamma \entails \lifdeltanovar{ Q_1 z_{t_1} \dots Q_i z_{t_i} \lifpart^*( \LIpre(C), T_i )} \lor \lifdeltanovar{C_\Gamma}\] for $i\varleq n$.

	Note that for $i=0$, 
	$\Gamma \entails \lifdeltanovar{ Q_1 z_{t_1} \dots Q_i z_{t_i} \lifpart^*( \LIpre(C), T_i )} \lor \lifdeltanovar{C_\Gamma}$ is nothing else than $\Gamma \entails \lifdeltanovar{\LIpre(C)} \lor \lifdeltanovar{C_\Gamma}$, which holds by assumption.

	For the induction step, suppose that $\Gamma \entails \lifdeltanovar{ Q_1 z_{t_1} \dots Q_i z_{t_i} \lifpart^*( \LIpre(C), T_i )} \lor \lifdeltanovar{C_\Gamma}$ holds.
	In $\lifpart^*( \LIpre(C), T_{i+1} )$, the term $t_{i+1}$ is lifted.
	As some lifting condition holds for $t_{i+1}$, $C$ does not contain $t_{i+1}$.
	%The quantifier prefix is extended by $Q_{i+1} z_{t_{i+1}}$, so the occurrences of $z_{t_{i+1}}$ are bound. \mytodo{vllt nicht benötigt?}
	We distinguish based on the color of $t_{i+1}$:
	\begin{itemize}
		\item
			Suppose that $t_{i+1}$ is a $\Delta$-term.
			Then the lifting variable $x_{t_{i+1}}$ occurs free in $\Gamma \entails \lifdeltanovar{ Q_1 z_{t_1} \dots Q_i z_{t_i} \lifpart^*( \LIpre(C), T_i )} \lor \lifdeltanovar{C_\Gamma}$.
			Note that is possible that an occurrence of the term $t_{i+1}$ is lifted and quantified in $\LI(C')$ for some predecessor $C'$ of $C$ and the occurrence of $t_{i+1}$ in $\LIpre(C)$ may be in the scope of that quantifier\footnote{See Example~\ref{exa:lemma_part_renaming} for an illustration.}. 
			However due to the renaming of lifting variables of step~\ref{lemma_part_renaming} of the lifting procedure of $\LI$, the lifting variable replacing the occurrence of $t_{i+1}$ in $\LIpre(C)$ can not be bound by any quantifier in $\LIpre(C)$.
			...
			As $C$ does not contain $t_{i+1}$, $\lifdeltanovar{C_\Gamma}$ does not contain $x_{t_{i+1}}$.
			Hence we may quantify $x_{t_{i+1}}$ universally as follows:
			\[\Gamma \entails \lifdeltanovar{ Q_1 z_{t_1} \dots Q_i z_{t_i} \forall x_{t_{i+1}} \lifpart^*( \LIpre(C), T_{i+1} )} \lor \lifdeltanovar{C_\Gamma}.\]
		...
		\item
			Otherwise $t_{i+1}$ is a $\Gamma$-term and the term $\lifdeltanovar{t_{i+1}}$ occurs in $\lifdeltanovar{\lifpart^*(  \LIpre(C), T_i )}$.
			By Lemma~\ref{lemma:lifting_conditions}, no subterm of $t_{i+1}$ is lifted and quantified in $\LIpre(C)$.
			Hence all $\Delta$-lifting variables in $\lifdeltanovar{t_{i+1}}$ either occur free 
			...
			the quantifier for $y_{t+1}$ is in the scope of the quantification of these.
			Therefore $\lifdelta{t_{i+1}}$ is a valid witness term for the existential quantification of $y_{t+1}$.
			%$\Gamma \entails \lifdeltanovar{ Q_1 z_{t_1} \dots Q_i z_{t_i} \exists y_{t_{i+1}} \lifpart^*( \LIpre(C), T_{i+1} )} \lor \lifdeltanovar{C_\Gamma}$. 
	\end{itemize}

	By this induction, we obtain that
	$\Gamma \entails \lifdeltanovar{ Q_1 z_{t_1} \dots Q_n z_{t_n} \lifpart^*( \LIpre(C), T_n )} \lor \lifdeltanovar{C_\Gamma}$, which is the same as
	$\Gamma \entails \lifdeltanovar{ \LI(C)} \lor \lifdeltanovar{C_\Gamma}$.
\end{proof}
