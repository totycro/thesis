...
\begin{proof}
	...
	\mytodo{ $s'$, $t'$: wieso sind sie eindeutig bestimmt, wieso existieren sie? }
...
\mytodo{\textsc{\LARGE Masterplan:} 

	von $\LI(C_i)$ auf $\LIpre(C)$ normal argumentieren.

	dann lemma dass von $\LIpre(C)$ auf $\LI(C)$ geht (weil das kommt im beweis im base case und im step vor)

	dieses lemma muss induktion sein.
	vllt invariante: wenn $t$ in $\LIpre(C)$ ODER $\LI(C)$, dann ist jeder subterm $s$ von $t$ nicht geliftet

	beweis: 
	\begin{itemize}
		\item 
			von $\LI(C_i)$ auf $\LIpre(C)$ wird nicht geliftet, da kann nichts schief gehen (was passiert bei substitution genau?)

		\item
			von $\LIpre(C)$ auf $\LI(C)$: 
			sei $t$ ein term und $s$ ein subterm von $t$.
			sei $t$ nicht geliftet in $\LI(C)$. dann zu zeigen dass $s$ auch nicht geliftet in $\LI(C)$.

			Supp $s$ geliftet wegen lift cond $i$. dann entsprechender widerspruch mit der nicht-liftung von $t$.
