...
A term $s$ is a specialisation of $t$ if there exists a substitution $\sigma$ such that $t\sigma = s$.
If $s$ is a specialisation of $t$, then $t$ is an abstraction of $s$. Note that abstraction and specialisation are reflexive.
...
A term is a $\Phi$-term if its outermost symbol is $\Phi$-colored.
\end{defi}
