...
The original publication in \cite{beth1953} precedes Craig's papers on interpolation (\cite{Craig57linear,Craig57three}) by four years and hence relies on a direct proof.
The definability theorem deals with definitions of predicates by means of formulas in first order logic and bridges the gap between implicit definitions, where predicates are defined by its use, and explicit definitions, which define a formula by means of another formula, by even showing their equivalence.
...
and by reducing $\Gamma_P$ and $\Gamma_{P'}$ to finite sets $\Gamma^*_P$ and $\Gamma^*_{P'}$ respectively by a suitable application of the compactness theorem can be expressed by the second-order sentence
$\forall P \forall P' (  (\bigwedge_{\varphi \in \Gamma^*_P \cup \Gamma^*_{P'}} \varphi ) \limpl P=P')$.
