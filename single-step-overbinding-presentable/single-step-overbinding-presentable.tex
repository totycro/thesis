...
\begin{proof}
	...
	As the quantifier prefix is ordered according to the arrows, Lemma~\ref{lemma:arrows_for_lifting_terms} gives the result. \mytodo{verify this after the proof of the lemma}
\end{proof}
...
\begin{defi}[Unification algorithm]
	The most general unifier $\mgu$ of two literals $l = A(s_1,\dotsc, s_n)$ and $l' = A(t_1,\dots, t_n)$ is defined to be $\mgu(\{ (s_1, t_1), \dotsc, (s_n, t_n)\})$.
		...
		Let $\id$ denote the identity function and $\textbf{fail}$ be returned by $\mgu$ in case the arguments are not unifiable.

		$\mgu$ for a set of pairs of terms $T$ is defined as follows:
		...
		$
		\mgu(\{t\} \cup T) = \begin{cases}
			\mathbf{fail} 				& \text{if $t = (x, s)$ or $t=(s,x)$ and $x$ occurs in $s$} \\
			\mgu(T\subst{x/s})\subst{x/s} \cup \{x\mapsto s\} 		& \text{if $t = (x, s)$ or $t=(s,x)$ and $x$ does not occur in $s$} \\
			\mathbf{fail} 				& \text{if $t = (f(s_1,\dotsc,s_n), g(s_1,\dotsc,s_n))$ with $f\neq g$} \\
			\mgu(T \cup \{(s_1, t_1), \dotsc, (t_n, s_n)\})		& \text{if $t = (f(s_1,\dotsc,s_n), f(t_1,\dotsc,t_n))$} \\
			\mgu(T) 							& \text{if $t=(s, s)$} \\
		\end{cases}
		$
\end{defi}
\begin{conj}
	A $\Delta$-colored term can only be a subterm of a $\Gamma$-colored term in a subsequent clause if there is a variable which has a $\Gamma$-colored occurrence and a grey occurrence in the current clause $C$.
\end{conj}
\begin{proof}
	Suppose no such variable exists in $\Gamma$ and $\Delta$.
	By the definition of the colors, no $\Gamma$-colored term initially contains a $\Delta$-colored term. We show that no resolution derivation can introduce one.
	...
	\begin{enumerate}
		\item
			\label{a5hsefdgsy6}
			Suppose a unifier $\sigma$ of two literals $l$ and $l'$ of $C_1$ and $C_2$ respectively introduces a $\Delta$-term $t$ in a $\Gamma$-term $s$ in $C_1$. 
			Then a variable $x$ occurs in $s$ such that $x\sigma = t$.
			By the unification algorithm, this implies that $x$ is directly unified with at least the outermost symbol of $t$.
			Note that clauses involved in a resolution step are variable disjoint, so $x$ only occurs in $C_1$. 
			...
			$\Gamma$- and $\Delta$-colored terms are not unifiable, so a unifier mapping $x$ to a $\Delta$-colored term must be created from a unification of $\Gamma$-terms.
			But by assumption, no $\Gamma$-colored term contains a $\Delta$-colored term.  So by unification of a $\Gamma$-colored term containing $x$ with another $\Gamma$-colored term, it is not possible that the $\mgu$ maps $x$ to a $\Delta$-colored term, which is a contradiction.


		\item
			Suppose a unifier $\sigma$ of two literals $l$ and $l'$ of $C_1$ and $C_2$ respectively introduces a $\Gamma$-term $s$, which contains a $\Delta$-term $t$.
			...
			This does not exclude the case that a variable $y$ is unified first with a $\Gamma$-term containing a variable $x$, where later $x$ is unified with a $\Delta$-term. 
...
\begin{conj}
	...
	Hence if they are present in a clause $C$ at some point in the derivation, their position in the original clause has been a variable or has been contained in a variable in case a varaiable has been added inside of a term.
\end{conj}

\mytodo{possibly show that only a variable with a grey and a colored occurrence introduce foreign terms and multiple colored occurrences don't introduce but propagate the terms. check which formulation the proof below would need.}
...
\begin{conj}
	...
	then there is an arrow from an occurrence of $x_i$ to $t$ in $\AI^\Delta(C)$ in $\arr''$.
\end{conj}
\begin{proof}
	...
	There are 2 cases, we show that in both of them, there is an arrow from an occurrence of $x_i$ to $t$ in $\arr''$
	\begin{itemize}
		\item $\sigma$ introduces the $\Delta$-colored term $\colterm{i}$ in $t\newterm$.
			Hence $t$ contains a variable $u$ such that $u\sigma = \colterm{i}$.

			\begin{itemize}
				\item
					Suppose $t$ does not occur in the literal being unified. 
					Then $u$ is only set due to occurring in the literal being unified, say at position $p$.

					The occurrence at $p$ is either grey or colored.
					As variables are never added (cf.~\ref{lemma:variable_never_added}) and arrows never removed, the original arrows still apply.
					$u$ in $t$ is a colored occurrence.
					
					Suppose $p$ is a grey occurrence. Then there is an arrow from $p$ to $u$ in $t$.

					Otherwise suppose $p$ is a colored occurrence. Then there is a weak connection between $p$ and $u$ in $t$.
					But as then the literal $p$ is unified to does not introduce the $\Delta$-term into a $\Gamma$-term, there has to be some kind of induction hypothesis which gives the result here.
				...
				\item
					...
					\begin{itemize}
						...
						\item[\markB]
							...
							We know that $t'$ is the lifting of an abstraction of $\colterm{i}$.
							We assume it is not a variable.
							Hence it is a maximal $\Gamma$-term.
							If it contains a lifting var just like $t^*$, we get the result by the induction hypothesis as there must be an arrow towards $t'$ and the arrows of the literals are being merged.

							Otherwise $t'$ does not contain a lifting variable.
							As $t$ does not either, but $t\sigma = t'\sigma = t^*$ does, 
							variables in $t$ or $t'$ or both are substituted for terms containing $\Delta$-terms.
							These variables occur elsewhere in the respective clauses.

							They occur either in colored terms as well.
							Then this is the same situation as this case (if not one of the fast bailouts above applies, in which case we are done).
							However there can only be finitely many such cases as formulas are finite and there cannot be a circle as the vars have proper substitutions, which must have originated somewhere.

							Otherwise these variables occur as outermost symbol. Then case \markA{} applies.
