...
\begin{enumerate}
	...
	\paragraph{Special cases.}
		...
		\hl{feasible} Unifier-induced arrow 1:\todo{necessary special case}{} 
		If $x$ occurs in $y\sigma$, add an arrow from every grey occurrence of $x$ in $C_i$ to every colored occurrence of $y$ in $C$.
		...
		\hl{feasible} Unifier-induced arrow 2:\todo{necessary special case}{} 
		If a lifting variable $z_i$ occurs in $x\sigma$, add an arrow from every grey occurrence of $z_i$ (?$C$ or $C_i$?) to every colored occurrence of $x$ in $C_i$.
	...
	\begin{quote}
		...
		Then add arrow from every grey occurrence of $z$ to every grey occurrence of $y$

		By merge case 4, have merge edge from grey occ of $x$ to grey occ of $z$.
		By merge case 3, have arrow from grey occ $x$ to every grey occ of $y$. 
	\end{quote}

	Backwards merge case 1':
	\begin{quote}
		Have $x$ occurs grey in $z\sigma$ and $x$ occurs grey in $y\sigma$

		Then arrow from grey occ of $z$ to colored occ of $y$. 

		By merge case 4, have merge edge from grey occ of $x$ to grey occ of $z$.
		By unifier-induced arrow 1, have arrow from $x$ to every colored occ of $y$.
	\end{quote}

	Backwards merge case 2:
	\begin{quote}
		Have $x$ occurs grey in $z\sigma$ and $x$ occurs colored in $y\sigma$

		Then add arrow from every grey occurrence of $z$ to every colored occurrence of $y$
	\end{quote}
...
\begin{cases}
	...
	\{x_j \mapsto x_m, x_k \mapsto x_m\} & \parbox[t]{.75\textwidth}{if $a\cl = x_j$ and $b\cl = x_k$, both lifting variables, and $x_m$ is the corresponding lifted term in the unified literal, i.e. $x_m = \lifboth{a\sigma} = \lifboth{b\sigma}$.\newline If the terms were lifting variables before and we do not know what the actual term is, choose any fresh lifting variable. \mytodo{last sentence is unconfirmed}
	{\tiny
