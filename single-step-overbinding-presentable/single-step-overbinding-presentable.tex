...
\begin{proof}
	...
	Suppose that $t_i$ does not contain a variable.
	Then any occurrence of $t_i$ is never altered throughout the derivation.
	If a substitution introduced $t_i$, it is lifted to $x_i$.
	By Lemma~\ref{lemma:tau_only_for_variable_terms}, $\tau$ does not alter $x_i$. 
	Hence all occurrences to $x_i$ refer to $t_i$ at any given stage of the derivation.

	Now we suppose that $t_i$ does contain a variable.
	...
	As $u\cl$ does not contain $x_i$, it was introduced either by $\tau$ or by $\sigma$ introducing $t_i$ which is then lifted to $x_i$.
	\begin{itemize}
		\item Suppose $r\cl$ was introduced by $\sigma$ and consecutive lifting. 
			Then $\sigma$ has introduced $t_i$.

			\mytodo{} Note that with ``strict variable renaming'', each variable is renamed every time so $t_i$ cannot be introduced here as the variable it would have to contain does not exist anymore.

			Without strict renaming, we can show that then an arbitrary occurrence of $x_i$ in $\AIclause(C)$ different from $r$ also refers to $t_i$.
			Note that $C_1$ and $C_2$ are variable disjoint.
			Hence the set of variables of $t_i$ is a subset of the variables of $C_1$. \mytodo{clarify on the conditions that are ncessary for this. if some other $t_i$ occurs in $C_1$, then no variable of $t_i$ must occur in $C_2$. But if only $t_i\rho$ occurs in $C_1$, $C_2$ might ``reintroduce'' a variable from $t_i$}

			Note furthermore that if any of the variables of $t_i$ would not be present in $C_1$ anymore, than $\sigma$ could not have produced $t_i$ as unifications never introduce new variables.
			...
			Hence no variable of $t_i$ has been unified in the derivation leading to $C_1$. But as term are only changed by means of unification, the $t_i$ which were responsible for introducing the other occurrences of $x_i$ in $\AIclause(C_1)$ are still present, and they are all equal to $t_i$.

		\item Suppose $r\cl$ was introduced by $\tau$.
			Then $(x_j \mapsto x_i) \in \tau$.

			By Lemma~\ref{lemma:lifting_var_refers_to_abstraction_of_term}, $r = t_i$ and either for a term $b\cl\in\AIclause(C_1)\cup\AIclause(C_2)$ it holds that either, depending on the type of $\tau$-substitution, $\lifboth{b\cl\sigma} = x_i$ and $b=b\cl$ or $a\sigma = b\sigma = x_i$.
			In both cases, the variables of $t_i$ are present in $b\sigma$, so either they have been present before (i.e.\ in $b$) or were introduced by $\sigma$. In any case, by a similar reasoning as in the former case, the other occurrences of $x_i$ in $\AIclause{C_1}$ refer to $t_i$ as well. 
			\qedhere
	\end{itemize}
\end{proof}
...
\begin{lemma}
	...
	Let $a\cl = z_j$ a lifting variable. Then $a = t_j\rho$ for some substitution $\rho$.
	Even more, if a substitution $z_j\mapsto z_k$ for lifting variables $z_j$ and $z_k$ occurs,
	$z_k$ refers to exactly $t_k$ and there exists a substitution $\rho'$ such that $t_k = t_j\rho'$.
	\comm{used}
\end{lemma}
\NB{this probably also hold in $\AImatrix$ and for terms not occurring $\AIclause$ as well.}
\begin{proof}
	Base case:
	$z_j$ is introduced to lift $t_j$, $\rho$ is the identity function.

	Induction step:
	Suppose $z_j$ refers to $t_j\rho$ for some $\rho$.

	Suppose $t_j\rho$ changes in the course of the resolution derivation.
	Then it changes by means of unification, say by the unifier $\sigma$.
	Then it changes to $t_j\rho\sigma$, so now $z_j$ refers to $t_j\rho\sigma$.
...
%	We show that if $z_j$ is changed to $z_k$, then $t_j\rho'\sigma = t_k$, hence by transitivity of the specialisation-relation, $t_k$ is still a specialisation of the original introduction.
	...
	\begin{itemize}
		\item Suppose \circled{1} kind of $\tau$ entry, $(z_j \mapsto z_k)$.
			...
			But then $t_k = b\cl\sigma = b\sigma = a\sigma = t_j\rho\sigma$.
		...
		\item Suppose \circled{2} kind of $\tau$ entry, $(z_j \mapsto z_k)$.\nopagebreak
			...
			Hence $t_j\rho \sigma = a\sigma = t_k$.
			\qedhere
	\end{itemize}
\end{proof}
...
But we can also show that the terms in $l$ only become more specialised, i.e.\ if a lifting variable $z_j$ occurs in $l\cl$, the corresponding term in $\lifboth{l}$ is a specialisation of $t_j$, 
...
\begin{lemma}[Strenghtening of \ref{lemma:literal_in_clause_similar}]
	...
	If $s\cl$ is a lifting variable $z_i$ and $\lifboth{s} = z_j$ (i.e.\ $s=t_j$),
...
\begin{conj}
	All lifting variables of a clause refer to the same term. If the var is $x_i$, it is $t_i\rho$ for some $\rho$
\end{conj}

\begin{lemma}
	\label{lemma:lifting_variables_disjoint}
	Let $t_k$ be a colored term containing a variable and $z_k$ its corresponding lifting variable.
	Then $z_k$ does not occur in both $\AIclause(C_1)$ and $\AIclause(C_2)$ if $C_1$ and $C_2$ are resolved.
\end{lemma}
\begin{proof}
	$C_1$ and $C_2$ are variable disjoint, so $t_k$ cannot occur in both the derivations leading to $C_1$ and $C_2$ respectively. \mytodo{check what happens if proof is not a tree derivation}
\end{proof}
	...
	\begin{description}
		\item[Lifting variables.]
			...
			\item W.l.o.g.~$s\fromclause = z_i$ and $t\fromclause$ is not a lifting variable.
				...
				As $t$ is a variable and due to $t\sigma = t_k$ and as $\sigma$ is the most general unifier, it is necessary to substitute $t_k$ in order to unify the literals. 
				...
				Hence one of these clauses originally contained $t_k$, as otherwise $z_k$ would not have been used (cf.\ Lemma~\ref{lemma:lifting_var_refers_to_abstraction_of_term}).
				
				By Lemma~\ref{lemma:lifting_var_refers_to_abstraction_of_term}, $z_l$ refers to precisely $t_l$. %and there exists a substitution $\rho''$ such that $t_k\sigma = t_l$.
				As $z_l$ replaces $z_k$, and $z_k$ used to refer to $t_k$, but as at the same position now $z_l$ refers to $t_l$, some sequence of substitutions occurred which changed $t_k$ to $t_l$. 
				This substitution however has substituted at least one variable of $t_k$.
				But as the set of clauses is unique for a clause, this variable does not occur in the consecutive derivation.

				As however $\sigma$ introduces $t_k$ and therefore all of its variabels as subterms, 
				$t_k$ has never been updated to $t_l$, but then $(z_k \mapsto z_l)$ is not an element of $\tau$.
				{

				\tiny
				Other attempt: 

				$t_i \rho = t_k$ for some $\rho$.

				By Lemma~\ref{lemma:lifting_var_refers_to_same_terms}, every occurrence of $z_k$ in both clauses refers to the same term, say $t_k\rho'$.


				But $t\sigma = t_k$. 

				If $\sigma$ would remove some variable present in $t_k$, $t_k$ would not unify with any other term, but $\sigma$ is a unifier. Hence $\sigma$ must not remove any free variable of $t_k$.

				As however $t_l$ is a specialisation of $t_k$, at some point, a unifier has removed some variable of $t_k$. However this would imply that the respective variable isn't present in the clause anymore and as all clauses are variable disjoint, it would not appear in the whole derivation anymore.
				...
				%By Lemma~\ref{lemma:just_introduced_lifting_vars_not_affected_by_tau}, $z_k\tau = z_k$.
