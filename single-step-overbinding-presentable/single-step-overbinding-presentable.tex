...
$u_1, \ldots, u_m$ are comprised of all $x_i$ and $y_i$ in $\AImatrix(C)$. \comm{there are free variables in $\AImatrix(C)$, which are ``implicitly'' universally quantified.}
...
\begin{proof}
	...
	\begin{enumerate}
		\item
			\label{a5hsefdgsy6}
			Suppose a unifier $\sigma$ of two literals $l$ and $l'$ of $C_1$ and $C_2$ respectively introduces a multicolored $\Gamma$-term $s$ in $C_1$. 
...
\begin{proof}
	...
	\begin{itemize}
		\item $\sigma$ introduces the $\Delta$-colored term $\colterm{i}$ in the $\Gamma$-term $t\newterm$.
			...
			\begin{itemize}
				\item
					Suppose $t\occur{u}$ does not occur in the literal being unified, say $l$. 
					Then $u$ is changed because of an occurrence in literal being unified.
					\markA{}

					\begin{itemize}
						\item
							Suppose the occurrence $u$ in the literal is grey.
							...
							As in $\AI(C)$ , $\colterm{i}$, which $\sigma$ introduces for $u$ is lifted to $x_i$, and there is an arrow from the occurrence at $r$ to the one in $t\occur{x_i}$.

						\item
							Otherwise suppose $u$ occurs in the maximal colored term $r\occur{u}$ in the literal.
							Then there is a weak connection between $r\occur{u}$ and $t$, so by construction, $r$ and $t$ share all arrows.
							...
							Otherwise suppose $r'$ is a $\Delta$-term. 
							Then $r\occur{u}$ is a $\Delta$-term as well.

							NOTE: $r\occur{u}$ is maximal colored and $t\occur{u}$ is maximal $\Gamma$-colored. As no $\Delta$-terms occur in $\AI(C_1)$, but $t\occur{u}$ occurs in $\AI(C_1)$, it is a maximal colored term.

							\mytodo{see example 210a/b, this is a problem}

							\mytodo{how to apply fix properly? have to argue that grey occurrences of different variabes have to be involved $\ra$ in which case there are arrows}
