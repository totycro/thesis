...
\begin{conj}
	...
	Let $\colterm{i}$ is contained in some literal in $\Gamma$ and
	a term $t$ derived from $\colterm{i}$ occurs in $\AI(C)$ as in Lemma~\ref{ref:gamma_entails_delta_terms_lifted}. 
	Suppose $\colterm{i}$ contains some $\Delta$-colored $\colterm{j}$.
	Then as $\Delta$-terms are lifted in $t$, it contains $z_j$.

	There is an arrow from some occurrence of $z_j$ in $\AI(C)$ to $\colterm{i}$.
...
\begin{conj}
	...
	If the corresponding lifting variables $z_i$ and $z_j$ occur in $\AI(C)$ for some $C$, then there is an arrow from an occurrence of $z_i$ to an occurrence of $z_j$.
