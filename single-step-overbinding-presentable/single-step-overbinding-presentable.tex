  ...
  \begin{description}
    \item{Resolution.}
      ...
      \end{prooftree}
			...
			$\Gamma \stackrel{\markA}\entails \AImatrix(C_1) \lor \clauseOnePrime \lor {(l{\fromclause})_\Gamma}$

			$\Gamma \stackrel{\markB}\entails \AImatrix(C_2) \lor \clauseTwoPrime \lor {\lnot (l{\fromclause'})_\Gamma}$
		...
		\begin{itemize}
			\item if $l$ and $l'$ grey:
				...
				Hence by Lemma~\ref{lemma:substitute_and_lift} and Lemma~\ref{lemma:lift_logic_commute}, we get that 

				$\Gamma \entails \lifboth{\AImatrix(C_1)\sigma} \lor \lifboth{\clauseOnePrime\sigma} \lor \lifboth{l{\fromclause}\sigma}$

				$\Gamma \entails \lifboth{\AImatrix(C_2)\sigma} \lor \lifboth{\clauseTwoPrime\sigma} \lor \lnot \lifboth{l{\fromclause'}\sigma}$
			...
			\item if $l$ and $l'$ are $\Gamma$-colored :
				...
				i.e.
				$\Gamma \entails 
				 \lifboth{\Big(\AImatrix(C_1) \lor \AImatrix(C_2)\Big)\sigma} \tau
												\spa\lor
			\lifboth{\Big( \clauseOnePrime \lor \clauseTwoPrime \Big)\sigma} \tau $
		...
		\end{itemize}
			...
			\begin{itemize}
				...
				$\AImatrix(C) =
				\Big((\lnot {\lifboth{l\sigma}} \land \lifboth{\AImatrix(C_1)\sigma} \lor
				(\lifboth{l\sigma} \land \lifboth{\AImatrix(C_2)\sigma) } \Big)\tau
				$
