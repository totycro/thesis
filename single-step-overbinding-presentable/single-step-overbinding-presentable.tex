	...
	%	\]
		...
		\begin{itemize}
			\item if $l$ and $l'$ don't have the same color:
				...
				$\AImatrix(C) =
				(\lnot {l^\circ}' \land \lifboth{\AImatrix(C_1)\sigma}\tau) \lor
				(l^\circ \land \lifboth{\AImatrix(C_2)\sigma) }\tau
				$
...
\begin{remark}[ad Lemma \ref{lemma:lift_subst_commute}]
	Lemma \ref{lemma:lift_subst_commute} does not hold in case $x_i$ occurs in $C$.
	This can easily be seen using a counterexample:
