...
\begin{proof}
	...
	Suppose that elsewhere in the literal $l\cl$ $s = z_k$ occurs and in $l\cl'$, there's $t$ at that position.

	$s\cl \sim \lifboth{l}$, so $s$ is some colored term.

	TODO

	\begin{itemize}
			\item
				Suppose that $(z_k\mapsto z_j) \in \tau$ (first kind). 
...
\begin{proof}
	...
	Remark on $\tau$: $\tau$ only replaces lifting terms by other lifting termlifting terms by other lifting terms
...
\begin{lemma}[Strenghtening of \ref{lemma:literal_in_clause_similar}]
	...
	If for term $s$ in $l$ and the corresponding $s\cl$ in $l\cl$ we have that $s\cl = z_i$ and $\lifboth{s} = z_j$ with $i\neq j$, 
