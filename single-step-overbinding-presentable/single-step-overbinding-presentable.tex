...
\begin{defi}[Unification algorithm]
	The most general unifier $\mgu$ of two literals $l = A(s_1,\dotsc, s_n)$ and $l' = A(t_1,\dots, t_n)$ is defined to be $\mgu(\{ (s_1, t_1), \dotsc, (s_n, t_n)\})$.
		...
		$ \begin{cases}
			\mathbf{fail} 				& \text{if $t = (x, s)$ or $t=(s,x)$ and $x$ occurs in $s$ but $x\neq s$ } \\
			\mgu(T\subst{x/s})\subst{x/s} \cup \{x\mapsto s\} 		& \text{if $t = (x, s)$ or $t=(s,x)$ and $x$ does not occur in $s$ or $x=x$} \\
...
\end{defi}
\begin{conj}
	A resolution derivation can only contain a $\Gamma$-colored term containing a $\Delta$-colored term if there is a variable which has a $\Gamma$-colored occurrence and a grey occurrence in a clause.
\end{conj}
\begin{proof}
	...
	\begin{enumerate}
		\item
			...
			\mytodo{PROBLEM: ``rampen'' can put gamma terms in delta terms, then they are unifiable with other delta terms (which still is useless, but i state the opposite below}
			...
			So by unification of a $\Gamma$-colored term containing $x$ with another $\Gamma$-colored term, it is not possible that the $\mgu$ maps $x$ to a $\Delta$-colored term, which is a contradiction.
