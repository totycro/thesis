...
\begin{frame}{Craig Interpolation (1/2)}
	...
	\emph{Let \myA{}
		...
		\begin{itemize}
			\item \myA{} contains \colA{red} and \mygray{mygray} symbols and
			\item \myB{} contains \colB{blue} and \mygray{mygray} symbols
		\end{itemize}
		such that:
		\begin{itemize}
		...
		\begin{tabular}{p{.4\textwidth}p{.5\textwidth}}
			\mbox{Then there is a interpolant $\mygray I$ containing only \mygray{mygray} symbols such that:}
			\begin{itemize}
	...
	\begin{example}
		\begin{itemize}
			\item Let $\Gamma = \{\colA P(\mygray a), \lnot \colA P(\mygray b)\}$ and $\Delta = \{ \mygray a \neq \mygray b \}$.
				\pause 
			\item Only possible interpolant: $\mygray a \neq \mygray b$
		\end{itemize}
	\end{example}
