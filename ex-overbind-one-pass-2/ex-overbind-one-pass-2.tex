...
\begin{prooftree}
	...
	\BinaryInfCm{ Q(g(z)) \mid P(f(g(z)), u) \lor R(u) \lor R(z) }
	\RightLabelm{ u\mapsto z}
	\UnaryInfCm{ Q(g(z)) \mid P(f(g(z)), z) \lor R(z) }

	\AxiomCm{ \sig{ S(h(u)) \lor \lnot P(f(u), c) } }
	\RightLabelm{ u\mapsto g(c), z\mapsto c}
	\BinaryInfCm{ P(f(g(c)), c) \mid Q(g(c)) \mid S(h(g(c))) \lor R(c) }
...
\begin{prooftree}
	...
	\AxiomCm{ \sig{ S(h(u)) \lor \lnot P(f(u), c) } }
	\BinaryInfCm{ P(f(x_{g(c)}), y_c) \mid Q(x_{g(z)}) \mid S(h(x_{g(z)})) \lor R(c) }

\end{prooftree}


\largered{can prove something here which doesn't work when both are lifted? don't think so}
