...
\e{501 -- like 502 but easier}

\begin{prooftree}
	\AxiomCm{ \forall x_a \exists y_{f(a)} Q(x_a, y_{f(a)})  \mid A }
	\AxiomCm{ \forall x_a( S(x_a)) \mid \lnot A }
	\BinaryInfCm{ \forall x_a( S(x_a)) \spam\land \forall x_a \exists y_{f(a)} Q(x_a, y_{f(a)})  \mid \square } 
\end{prooftree}
\begin{center}
	no first order operation in this last inference $\Ra$ nothing to prove
\end{center}

\e{502 -- example with multiple, independent $a$'s}
...
\end{center}
\begin{prooftree}
	\AxiomCm{ \sig{ P(f(x), x) \lor Q(z) \lor R(z) }}
	\AxiomCm{ \p{ \lnot R(a) }}

	\BinaryInfCm{ R(a) \mid P(f(x), x) \lor Q(a)}

	\AxiomCm{ \p{ \lnot Q(u) }}

	\BinaryInfCm{ Q(a) \lor R(a) \mid P(f(x), x)}

	\AxiomCm{ \p{ \lnot P(z, a) }}

	\BinaryInfCm{ P(f(a), a) \lor Q(a) \lor R(a) \mid \square }
\end{prooftree}

~

\begin{center}
	invariant: $\lifdeltanovar{\LI(C)} \mid \lifdeltanovar{C}$
\end{center}
\begin{prooftree}
	\AxiomCm{ \sig{ P(f(x), x) \lor Q(z) \lor R(z) }}
	\AxiomCm{ \p{ \lnot R(x_a) }}

	\BinaryInfCm{ R(x_a) \mid P(f(x), x) \lor Q(x_a)}

	\AxiomCm{ \p{ \lnot Q(u) }}

	\BinaryInfCm{\forall x_a( Q(x_a) \lor R(x_a)) \mid P(f(x), x)}

	\AxiomCm{ \p{ \lnot P(z, x_a) }}


	\BinaryInfCm{\forall x_a \exists y_{f(a)} \Big( P(y_{f(a)}, x_a) \lor \forall x_a (Q(x_a) \lor R(x_a))  \Big) \mid \square}
\end{prooftree}
...
Detailed derivation:
	...
	$\Gamma \entails \forall x_a( Q(x_a) \lor R(x_a)) \mid P(f(x), x)$

	$\Gamma \entails P(z, x_a)$

	hence

	$\Gamma \entails \forall x_a \Big( {\forall x_a( Q(x_a) \lor R(x_a)) \mid P(f(x_a), x_a)}\Big)$

	$\Gamma \entails \forall x_a P(f(x_a), x_a)$

	hence

	$\Gamma \entails \forall x_a \Big( {\forall x_a( Q(x_a) \lor R(x_a)) \mid P(f(x_a), x_a)}\Big) \spal\land \forall x_a P(f(x_a), x_a)$

	hence

	$\Gamma \entails \forall x_a \Big( \forall x_a( Q(x_a) \lor R(x_a)) \mid P(f(x_a), x_a) \spal\land  P(f(x_a), x_a)\Big)$

	simplified 

	$\Gamma \entails \forall x_a \Big( \forall x_a( Q(x_a) \lor R(x_a))  \spal\land  P(f(x_a), x_a) \Big)$
...
\begin{prooftree}
	...
	\BinaryInfCm{ Q(a) \lor R(a) \mid P(f(x), x)}
	\noLine
	\UnaryInfCm{\forall x_a (Q(x_a) \lor R(x_a)) \mid P(f(x), x)}

	\AxiomCm{ \p{ \lnot P(z, a) }}

	\BinaryInfCm{ P(f(a), a) \lor \forall x_a (Q(x_a) \lor R(x_a)) \mid \square }
	\noLine
	\UnaryInfCm{\forall x_a \exists y_{f(a)} \Big( P(y_{f(a)}, x_a) \lor \forall x_a (Q(x_a) \lor R(x_a))  \Big) \mid \square}
\end{prooftree}
